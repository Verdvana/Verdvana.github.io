...
> 图片生成 Prompt：  
> 画一张“以太网硬件与芯片设计学习路线图”。从左到右依次为 Ethernet Frame、MAC TX/RX、GMII/RGMII/SGMII/XGMII、PCS、FEC、PMA、SerDes、PHY、Switch ASIC Pipeline。风格为现代芯片架构图，蓝色科技风，清晰标注模块名称，适合技术博客首图。
...
> 图片生成 Prompt：  
> 画一张 Ethernet MAC 模块框图。左侧是 internal packet stream interface，中央分为 MAC TX、MAC RX、CRC32、Pause Control、Statistics 五个子模块，右侧连接 GMII/RGMII/XGMII PHY Interface。使用芯片模块框图风格，突出 TX/RX 双向路径。
...
> 图片生成 Prompt：  
> 画一张 Ethernet CRC32 硬件计算示意图。左侧输入为 packet data bus，带 data、valid、sop、eop、keep 信号；中间是 parallel CRC32 engine；右侧输出 FCS 和 crc_error。图中标注 CRC 计算范围从 Destination MAC 到 Payload/Padding，不包括 Preamble、SFD 和 FCS。
...
> 图片生成 Prompt：  
> 画一张 MAC-PHY 接口演进图。从 MII、RMII、GMII、RGMII 到 SGMII、XGMII、CGMII，横轴表示速率从 100M 到 100G，纵向用小框展示每种接口的数据宽度、是否 DDR、是否串行。技术博客风格，简洁清晰。
...
> 图片生成 Prompt：  
> 画一张高速以太网 PHY 分层图。自上而下为 MAC、Reconciliation Sublayer、PCS、FEC、PMA、PMD、SerDes、Optical Module/Copper Cable。右侧标注每层职责：编码、纠错、串并转换、均衡、CDR、物理介质。风格为半导体芯片架构教学图。
...
> 图片生成 Prompt：  
> 画一张 400G Ethernet 多 lane 架构图。一个 400G MAC 连接到 PCS/FEC，然后拆分成 8 条 50G PAM4 SerDes lane，每条 lane 经过 PMA/PMD 到光模块。图中标注 lane alignment、FEC、PAM4、CDR。风格为数据中心高速网络芯片示意图。
...
> 图片生成 Prompt：  
> 画一张 Ethernet MAC RTL 实现路线图。五个阶段从左到右：8-bit MAC TX、8-bit MAC RX、VLAN/Jumbo、宽数据总线、FPGA PHY 实验。每个阶段下面用小图标表示 FSM、CRC、FIFO、CDC、Wireshark。风格为学习路径信息图。
...
> 图片生成 Prompt：  
> 画一张 Switch ASIC packet pipeline 图。左侧多端口 RX SerDes 输入，经过 PCS/FEC、MAC RX、Parser、VLAN、L2/L3 Lookup、ACL、QoS、Packet Buffer、Queue Manager、Scheduler、Egress Edit、MAC TX、PCS/FEC，右侧多端口 TX SerDes 输出。风格为高端交换芯片内部架构图，清晰、横向流程。
...
> 图片生成 Prompt：  
> 画一张 Switch ASIC buffer/queue/scheduler 架构图。多个 ingress port 的 packet 被切成 cell 存入 shared packet buffer，descriptor 进入多个 queue，scheduler 从 queue 中选择 packet，最后发往 egress port。图中标注 free list、admission control、WRED、SP/WRR/DRR scheduler。
...
> 图片生成 Prompt：  
> 画一张以太网芯片 bring-up 调试层次图。从 MAC internal loopback、PHY digital loopback、SerDes loopback、external cable loopback 到 live traffic test，逐层向外扩展。旁边列出关键观测指标：link up、block lock、FEC error、CRC error、packet counter、queue depth。
