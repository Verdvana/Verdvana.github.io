...
帮我写一个python3脚本，能够从xls或xlsx表格中获取数据，具体情况如下。表格通过命令输入。首先从名为pt或sync的sheet中获取如下数据：以ptsheet为例：内容为Signal Driving Cell的单元格右边的第一个单元格的内容存入pt.signal_driving_cell.name，右边第二个单元格的内容存入pt.signal_driving_cell.output，右边第三个单元格的内容存入pt.signal_driving_cell.input，右边第四个单元格的内容存入pt.signal_driving_cell.lib_name；内容为Clock driving Cell的单元格右边的第一个单元格的内容存入pt.clock_driving_cell.name，右边第二个单元格的内容存入pt.clock_driving_cell.output，右边第三个单元格的内容存入pt.clock_driving_cell.input，右边第四个单元格的内容存入pt.clock_driving_cell.lib_name；Signal Transition右边第一个单元格的内容存入pt.signal_trans.min，右边第二个单元格内容存入pt.signal_trans.max；Output Load右边第一个单元格的内容存入pt.output_load.min，右边第二个单元格内容存入pt.output_load.max；Setup Matgin右边第一个单元格内容存入pt.setup_margin；Hold Margin（ns）右边第一个单元内容存入pt.hold_margin；与Clock Tree Budget同为一列且在其下边的单元格内容为SM的右边第一个单元格内容存入pt.ctb.sm.source.min，右边第二个单元格存入pt.ctb.sm.source.max，右边第三个单元格存入pt.ctb.sm.network.min，右边第四个单元格存入pt.ctb.sm.network.max，右边第五个单元格存入pt.ctb.sm.trans.min，右边第六个单元格存入pt.ctb.sm.trans.max，右边第七个单元格存入pt.ctb.sm.skew，右边第八个单元格存入pt.ctb.sm.noise。名为sync的sheet格式一样，同理。名为clock的sheet要从最左侧单元格内容为Level的下一行开始读取，每一行的左侧十个单元格作为一个list，以此类推。
