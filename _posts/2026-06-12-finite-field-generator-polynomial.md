...
到这里，前文已经把校验过程解释成“用 $g(x)$ 定义合法码字集合，并用整除关系检查错误”。接下来还需要补上一层结构：这些码字不仅可以用多项式描述，而且和“移位”有天然关系。因为在多项式表示里，左移一位就相当于乘以 $x$。

如果只做普通左移，最高位会被移出固定长度的码字；如果把这个最高位再绕回最低位，就得到循环移位。循环码关注的正是这种结构：在固定长度下，合法码字经过循环移位后仍然落在同一类码字结构里。这个性质把“多项式整除”和“移位寄存器实现”连接起来，也为后面理解循环校验码中的“循环”打下基础。
