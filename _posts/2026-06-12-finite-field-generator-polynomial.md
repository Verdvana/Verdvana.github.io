...
到这里，前文已经把校验过程解释成“用 $g(x)$ 定义合法码字集合，并用整除关系检查错误”。接下来还需要补上一层结构：校验计算为什么会自然地出现移位，以及这种移位为什么会进一步引出循环结构。

校验不是把所有 bit 混成一个无序集合，而是要保留 bit 的先后位置。按顺序读入数据时，每来一个新 bit，已经读过的内容就要在状态里“往前挪一格”，给新 bit 腾出位置。状态宽度可以保持不变，但各个 bit 所处的位置和权重会改变，这就是移位的意义。
