...
到这里，前文已经把校验过程解释成“用 $g(x)$ 定义合法码字集合，并用整除关系检查错误”。但 CRC 里的 C 是 cyclic，也就是“循环”。这个词不是额外装饰，而是在说这类码字集合还有一个结构性质：在固定长度下，如果一个码字合法，那么它的循环移位仍然落在同一类码字结构里。

这个性质把“多项式整除”和“移位寄存器实现”连接起来。它让码字集合可以用多项式环里的理想来描述，也让工程实现能够自然地落到移位、反馈和异或网络上。不过这里不必一开始就陷入抽象代数，先抓住移位和乘以 $x$ 的关系就够了。
