...
位串为什么可以看成多项式？原因是我们可以把一串比特解释为多项式的系数。例如比特串 `101101` 可以写成：
...
生成多项式到底生成了什么？$g(x)$ 是循环校验码的核心参数。它不是随便挑出来的魔法常数，而是定义了哪些多项式可以被视为合法码字。

在一个具体系统里，$g(x)$ 通常不是由每条消息临时算出来的，而是在设计编码方案或制定协议时预先选定。发送端和接收端必须使用同一个 $g(x)$，否则一个端构造出来的合法码字，另一个端可能会判成非法。至于应该选哪个 $g(x)$，要看校验位宽、最大消息长度、目标错误模型和兼容要求；这一点会在后面的多项式选择和工程约束里继续展开。

### 3.1 从“附加校验位”到“构造合法码字”

在 $g(x)$ 已经选定的前提下，设原始消息多项式为 $m(x)$，生成多项式的次数为 $r$。如果要附加 $r$ 个校验比特，第一步通常是把消息左移 $r$ 位：
...
为了避免一开始就陷入 32 位多项式，可以先看一个很小的例子。这里的生成多项式不是推荐用于真实协议的选择，而是一个便于手算的玩具参数。选择它时至少要满足几个直观条件：次数为 $3$，这样余数正好是 $3$ 位；最高项系数为 $1$，这样它确实是三次多项式；常数项为 $1$，这样它不会只是在做简单移位；中间再保留一个非零项，让反馈关系不是退化的。
...
余数长度为什么等于校验位宽？如果生成多项式 $g(x)$ 的次数是 $r$，那么任何多项式除以 $g(x)$ 后，余数次数都小于 $r$：
...
循环结构为什么重要？“循环”这个词不是修饰语。循环码有一个特殊性质：如果一个码字合法，那么它的循环移位仍然合法。这让码字集合可以用多项式环里的理想来描述，不过工程上不必一开始就陷入抽象代数。
...
从代数走向实现时会遇到哪些约束？一旦进入工程实现，核心问题就从“能不能算”变成“怎样在给定资源下算得足够快、足够稳定、足够容易验证”。
...
CRC-32/IEEE 的正常多项式表示常写作 `0x04C11DB7`。由于最高项 $x^{32}$ 是隐含的，这个十六进制值对应的完整生成多项式是：
