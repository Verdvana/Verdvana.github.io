...
```
Create a professional, clean digital IC block diagram (hardware schematic / RTL architecture style) of a "Smart MMU (Flexible Shared SRAM Address Management Module)". White background, vector-crisp lines, engineering documentation style like a chip datasheet block diagram. Orthogonal (Manhattan) routing only — all wires are horizontal or vertical with 90-degree bends, NO diagonal lines, NO crossing or tangled wires; route wires around blocks with clear spacing.

OUTER BOUNDARY:
Draw a large rounded rectangle labeled "Smart MMU (clk_core 300MHz, single clock domain)". Outside the boundary place three external entities as small boxes:
- "QM (Enqueue/Dequeue Engine)" on the LEFT (the ONLY data/control peer).
- "Next-Ptr SRAM 8192x15bit" on the BOTTOM.
- "CPU / APB Host" on the RIGHT, and "IPS/MAC side" on the TOP-LEFT.

INTERNAL SUB-BLOCKS (place as labeled rectangles, grouped by plane, each a different soft fill color):
Control plane (top): "Enqueue Ctrl (+full-frame-drop FSM)", "Dequeue Ctrl (+backpressure)", "Recycle Ctrl (+multicast check)".
Storage-access plane (center): "Link-List Engine (LLE) — Access Arbiter, free_head/tail, q_head/q_tail[34], RMW bypass" (make this the central, largest, highlighted block).
Decision/aux plane (right-center): "Occupancy & Pool Mgr (counters, dual-pool, 3-level watermark, WRED, PAUSE)", "Multicast Ref-Count Mgr".
Config plane (bottom-right): "CSR / Stats (APB regs)" and "Init FSM".

SIGNAL LINE STYLE (follow EE schematic conventions strictly):
- Single-bit signals: THIN solid lines.
- Multi-bit buses: THICK lines with a small diagonal slash and a bit-width number label (e.g. "/13", "/15", "/6", "/4", "/3").
- Every line has an ARROWHEAD showing direction.
- Color-code by function group, rich but distinct palette:
  * Enqueue path = BLUE
  * Dequeue path = GREEN
  * Recycle path = ORANGE
  * Next-Ptr SRAM access = PURPLE
  * APB/config = GRAY
  * Flow-control / PAUSE / IRQ = RED
- Put a small LEGEND box (bottom-left) explaining: thin=1bit, thick+slash=bus, and the color meanings.

EXTERNAL-TO-INTERNAL connections (clearly show which sub-block each top-level pin goes to):
From QM (left) → Enqueue Ctrl: enq_req(1), enq_queue_id(/6), enq_port_id(/2), enq_is_mcast(1), enq_color(/3), enq_mcast_bitmap(/4), enq_sof(1), enq_eof(1).
Enqueue Ctrl → QM: enq_ready(1), alloc_valid(1), alloc_cell_addr(/13), alloc_drop_ind(1), alloc_sram_flag(1), alloc_pkt_head(1), alloc_pkt_tail(1), alloc_full_frame_drop(1).
From QM → Dequeue Ctrl: deq_req(1), deq_queue_id(/6), deq_backpressure(/4).
Dequeue Ctrl → QM: deq_ready(1), deq_cell_valid(1), deq_cell_addr(/13), deq_pkt_head(1), deq_pkt_tail(1).
From QM → Recycle Ctrl: recycle_req(1), recycle_cell_addr(/13), mcast_recycle_req(1), mcast_recycle_addr(/13).
Recycle Ctrl → QM: recycle_ack(1).
LLE ↔ Next-Ptr SRAM (bottom, PURPLE): nptr_ce(1), nptr_we(1), nptr_addr(/13), nptr_wdata(/15) downward; nptr_rdata(/15) upward.
CPU/APB (right, GRAY) ↔ CSR/Stats: psel,penable,pwrite(1 each), paddr(/16), pwdata(/32) in; prdata(/32), pready,pslverr(1) out.
Occupancy & Pool Mgr → IPS/MAC side (top, RED): pause_req(/4).
CSR/Stats → CPU (RED): irq_alarm(1), irq_aging(1), overflow_alarm(1), underflow_alarm(1).
Top-level clk/reset to all: clk_core(1), rst_core_n(1), init_start(1) → Init FSM; init_done(1) out.

INTERNAL inter-block connections:
- Enqueue Ctrl ↔ LLE: "alloc/append request" + "alloc result".
- Dequeue Ctrl ↔ LLE: "walk request" + "cell stream".
- Recycle Ctrl ↔ LLE: "push/free request".
- Occupancy & Pool Mgr → Enqueue Ctrl: "drop/accept decision (incl. WRED)".
- Recycle Ctrl ↔ Multicast Ref-Count Mgr: "ref_count-- / zero check".
- LLE → Occupancy & Pool Mgr: "alloc/free events (counter update)".
- CSR/Stats ↔ all blocks: thin gray config/status taps.

LAYOUT: keep generous routing channels between blocks so no wire overlaps; align pins on block edges; left edge = QM bus, bottom edge = SRAM bus, right edge = APB, top edge = PAUSE/IRQ. Add a title at top: "Smart MMU — Hardware Block Diagram". Professional, readable, high-resolution.
```
