...
&#160; &#160; &#160; &#160; 例如，一个两倍频时钟驱动的触发器发起数据，再由一倍频时钟驱动的触发器捕获数据，就是真实路径（real path）。如果设计人员将时钟同步器明确放置在两个时钟域之间，即使好像存在从一个时钟域到另一个时钟域的时序路径，但数据没有被约束要在一个时钟周期内通过同步器传播，因此这样的路径被称为伪路径（false path），因为是由时钟同步器来确保数据正确地跨时钟域传播。可以使用set_false_path命令指定时钟域之间的伪路径。
