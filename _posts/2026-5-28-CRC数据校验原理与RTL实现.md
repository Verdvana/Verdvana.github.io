...
> 图片生成 prompt：  
> 画一张中文数字芯片学习路线图，主题为“CRC 数据校验学习路径”。画面从左到右分为五个模块：GF(2) 多项式基础、CRC 参数模型、串行 LFSR、并行 CRC 展开、512b packet 级生成与校验。风格为现代扁平化技术插图，蓝绿色配色，包含数据包、XOR 门、LFSR 寄存器、流水线级、checker 标记等元素，适合技术博客横幅图。
...
> 图片生成 prompt：  
> 画一张 CRC polynomial 选择决策图，包含 packet length、错误模型、兼容协议、硬件开销、Hamming Distance 五个决策节点。图中展示 CRC-8、CRC-16、CRC-32、CRC-32C、CRC-64 的选择分支。风格为清晰的工程架构图，白底，蓝色节点，橙色重点标注，适合中文技术博客。
...
> 图片生成 prompt：  
> 画一张 CRC-32 LFSR 硬件结构图。包含 32 个触发器组成的移位寄存器、输入 data bit、feedback XOR、多个 polynomial tap。标注 LSB-first、右移、reflected polynomial 0xEDB88320。风格为数字电路框图，线条清晰，适合放在技术博客中。
...
> 图片生成 prompt：  
> 画一张 512-bit 并行 CRC32 流水线架构图。输入为 AXI-Stream tdata[511:0]、tkeep[63:0]、tvalid、tlast。数据被拆成 8 个 64-bit lane，经过 stage0 输入寄存、stage1 lane0-1、stage2 lane2-3、stage3 lane4-5、stage4 lane6-7、stage5 final xor 输出。标注 crc_state、seed、invert、valid pipeline、last pipeline。风格为现代数字 IC 数据通路框图。
...
> 图片生成 prompt：  
> 画一张 CRC32 packet 级生成与校验数据流图。左侧为发送端：payload 进入 CRC generator，输出 crc32，packetizer 将 payload 和 crc32 拼接。右侧为接收端：parser 分离 payload 和 received crc，CRC checker 重新计算并比较，输出 pass/fail；同时画出 residue 法的旁路：payload+crc 一起进入 checker。风格为芯片系统架构图，中文标注。
