...
![以太网学习路线图 - 图片占位](../assets/images/placeholder.png)

> **image2 prompt：**  
> 生成一张中文技术博客风格的“以太网基础学习路线图”。画面采用横向流程图，从左到右依次为：网络分层、以太网帧、MAC地址、ARP、交换机转发、VLAN、STP/LACP/QoS、Wireshark抓包、MAC/PHY硬件、Switch芯片架构。每个节点使用简洁图标，例如分层模型、数据帧、网卡地址、问答气泡、交换机、标签、环路保护、放大镜抓包、芯片、数据中心交换芯片。风格清晰、扁平化、浅色背景、适合技术博客插图，中文标注，16:9。
...
![OSI模型与以太网位置 - 图片占位](../assets/images/placeholder.png)

> **image2 prompt：**  
> 生成一张中文技术博客插图，展示 OSI 七层模型和以太网所在位置。左侧是七层竖向堆叠：应用层、表示层、会话层、传输层、网络层、数据链路层、物理层。右侧突出标注“以太网主要覆盖 L2 数据链路层和 L1 物理层”，并用箭头连接到 MAC 子层、PHY、网线/光纤。风格简洁专业，浅色背景，适合网络基础教程。
...
![MAC与PHY关系 - 图片占位](../assets/images/placeholder.png)

> **image2 prompt：**  
> 生成一张中文技术插图，展示 Ethernet MAC 与 PHY 的关系。图中从左到右依次为：CPU/SoC 或 FPGA、Ethernet MAC、接口总线 GMII/RGMII/SGMII/XGMII、PHY 芯片、RJ45 网口或光模块、网线/光纤。用箭头表示数据流双向传输，并标注“MAC负责帧处理”“PHY负责物理信号转换”。风格清晰、工程图风格、白色背景。
...
![Ethernet II帧格式 - 图片占位](../assets/images/placeholder.png)

> **image2 prompt：**  
> 生成一张中文技术博客插图，展示 Ethernet II 帧格式。使用横向分段色块表示字段：Preamble 7B、SFD 1B、Destination MAC 6B、Source MAC 6B、EtherType 2B、Payload 46-1500B、FCS 4B。每个字段下方用中文简短解释，例如“同步前导码”“帧开始标志”“目的地址”“源地址”“上层协议类型”“数据负载”“CRC校验”。要求清晰、扁平化、适合嵌入 Markdown 博客。
...
![ARP请求与响应流程 - 图片占位](../assets/images/placeholder.png)

> **image2 prompt：**  
> 生成一张中文网络教程插图，展示 ARP Request 和 ARP Reply 流程。画面中有主机A、交换机、主机B。步骤1：主机A广播 ARP Request，目的 MAC 为 FF:FF:FF:FF:FF:FF，询问“谁是 192.168.1.20”。步骤2：交换机向局域网内泛洪。步骤3：主机B单播 ARP Reply，回复自己的 MAC 地址 BB:BB:BB:BB:BB:BB。用箭头和编号标注，风格清晰直观。
...
![交换机MAC学习与转发 - 图片占位](../assets/images/placeholder.png)

> **image2 prompt：**  
> 生成一张中文技术插图，展示二层交换机 MAC 地址学习与转发过程。中间是一台交换机，连接 PC-A、PC-B、PC-C。步骤1：PC-A 从 Port1 发帧，交换机学习 AA:AA -> Port1。步骤2：目的 MAC 为 BB:BB，若表中未知则向 Port2/Port3 泛洪。步骤3：PC-B 回复后，交换机学习 BB:BB -> Port2。步骤4：后续 A 到 B 直接单播转发到 Port2。用 MAC 表小表格显示学习结果，风格清晰。
...
![802.1Q VLAN Tag格式 - 图片占位](../assets/images/placeholder.png)

> **image2 prompt：**  
> 生成一张中文技术插图，对比普通 Ethernet II 帧和带 802.1Q VLAN Tag 的帧。上方是不带 VLAN 的帧：Destination MAC、Source MAC、EtherType、Payload、FCS。下方是在 Source MAC 和 EtherType 之间插入 4 字节 VLAN Tag，并展开 TPID、PCP、DEI、VID 字段。使用不同颜色突出 VLAN Tag，中文标注，简洁专业。
...
![STP阻塞冗余链路 - 图片占位](../assets/images/placeholder.png)

> **image2 prompt：**  
> 生成一张中文网络基础插图，展示三台交换机组成三角形环路。左图标注“存在二层环路，广播帧循环导致广播风暴”；右图展示 STP 生效后，其中一条链路被标记为 Blocking，形成无环树形拓扑。用红色箭头表示广播风暴，用绿色箭头表示正常转发路径，中文标注，适合博客教学。
...
![QoS队列与调度 - 图片占位](../assets/images/placeholder.png)

> **image2 prompt：**  
> 生成一张中文技术插图，展示以太网交换机 QoS 队列和调度。左侧多种流量进入交换机：语音、视频、普通数据、备份流量。中间根据优先级进入 Queue0、Queue1、Queue2、Queue3。右侧 Scheduler 按 SP/WRR/DRR 等策略选择队列输出到端口。标注 PCP、Queue、Scheduler、Shaper。风格简洁、适合网络芯片和网络协议教程。
...
![Switch ASIC数据通路 - 图片占位](../assets/images/placeholder.png)

> **image2 prompt：**  
> 生成一张中文技术博客插图，展示简化 Switch ASIC 数据通路。横向流水线从左到右为：RX SerDes、PCS、MAC RX、Parser、VLAN/L2 Lookup/ACL/QoS、Packet Buffer、Queue、Scheduler、Egress Modify、MAC TX、PCS、TX SerDes。用芯片内部模块框图风格绘制，突出 packet metadata 和 payload 两条路径，中文标注，适合硬件工程师阅读。
...
![Wireshark以太网帧解析 - 图片占位](../assets/images/placeholder.png)

> **image2 prompt：**  
> 生成一张仿 Wireshark 风格的中文教学插图，不要使用真实软件截图。画面分为三层：上方是 packet list，中间是 protocol details tree，下方是 hex bytes。重点展开 Ethernet II，显示 Destination MAC、Source MAC、Type: ARP/IPv4、Frame Check Sequence。用高亮框标出以太网头部字段。风格接近抓包分析界面，但为原创示意图，清晰适合博客。
...
![以太网分阶段学习路径 - 图片占位](../assets/images/placeholder.png)

> **image2 prompt：**  
> 生成一张中文学习路径阶梯图，主题为“以太网学习四阶段”。第一级：基本概念，包括 Ethernet、MAC、Frame、ARP。第二级：核心二层协议，包括 VLAN、STP、LACP、QoS。第三级：抓包实践，包括 Wireshark、ARP、Ping、TCP/UDP、VLAN Tag。第四级：硬件深入，包括 MAC/PHY、SerDes、PCS/FEC、Switch ASIC。采用阶梯上升视觉，技术博客风格，清晰中文标注。
