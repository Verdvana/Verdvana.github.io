...
module lle_tb;
    ...
    task automatic print_status(string tag);
        ...
        $display("  free_head_next=%0d", u_dut.free_head_next_q);
        for (qi = 0; qi < QUEUE_NUM; qi++) begin
            if (u_dut.q_cell_cnt_q[qi] != 0)
                $display("  q[%0d]: head=%0d tail=%0d cnt=%0d head_next=%0d ph=%0b pt=%0b",
                         qi, u_dut.q_head_q[qi], u_dut.q_tail_q[qi], u_dut.q_cell_cnt_q[qi],
                         u_dut.q_head_next_q[qi], u_dut.q_head_ph_q[qi], u_dut.q_head_pt_q[qi]);
        end
...
module lle #(
    ...
    always_ff @(posedge clk_core or negedge rst_core_n) begin
        if (!rst_core_n) begin
            ...
            do_push = lle_free_req & ~rcy_fifo_full & ~build_active;
            do_pop  = rcy_grant;
