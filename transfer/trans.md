...
module lle_tb;
    ...
    //   ★ 反压建模: enq 受 lle_alloc_ready 反压。当 deq 正在走链 (占 SRAM) 时
    //     lle_alloc_ready=0, enq 被暂停。本 task 仅在 enq 真正落地
    //     (lle_alloc_evt=enq_grant=1) 时推进 ei, 模拟真实 enqueue_ctrl 在
    //     ready=0 时保持当前 cell、不丢 cell 的行为。
    //========================================================================
    task automatic enq_and_deq_concurrent(input int enq_qid, input int enq_cells,
        ...
        @(negedge clk);  // queue_id 生效, lle_qhead/q_empty 稳定

        while (ei < enq_cells || !deq_done) begin
            //----------------------------------------------------------------
            // negedge: 采样 + 驱动 (DUT 在下个 posedge 采样)
            //----------------------------------------------------------------
            // ---- enq 驱动: 一直发当前 cell (ei), 受 ready 反压 ----
            //   即使 ready=0 也驱动 fire (模拟 QM 持续 valid), DUT 用 ready 反压;
            //   是否推进 ei 由本拍 enq 是否真正落地 (lle_alloc_evt) 决定。
            if (ei < enq_cells) begin
                lle_alloc_fire_r     = 1'b1;
            ...
            // ---- deq 驱动 ----
            deq_fire_now = (!deq_done && !lle_q_empty);
            deq_tail_now = deq_fire_now & lle_qhead_pkt_tail;
            if (deq_fire_now) begin
                lle_deq_fire_r = 1'b1;
                $display("[%0t]   CONC DEQ: addr=%0d pt=%0b | enq ei=%0d ready=%0b alloc_evt(prev)=%0b",
                         $time, lle_qhead, lle_qhead_pkt_tail, ei, lle_alloc_ready, lle_alloc_evt);
            end
            else begin
                lle_deq_fire_r = 1'b0;
            end
            ...
            //----------------------------------------------------------------
            // 经过 posedge: DUT 处理本拍 enq/deq
            //----------------------------------------------------------------
            @(posedge clk);
            #1;  // NBA 稳定后读 DUT 组合输出

            // ---- enq 落地确认: lle_alloc_evt = enq_grant (本 posedge 真正入队) ----
            if (lle_alloc_evt) begin
                $display("[%0t]   CONC ENQ landed: cell ei=%0d", $time, ei);
                ei++;             // 仅在真正落地时推进
            end
            // ---- deq 落地确认: 本拍 deq_fire 且非空, 队头已出 ----
            if (deq_tail_now) deq_done = 1;
            if (!deq_done && deq_fire_now == 0 && lle_q_empty) deq_done = 1;
        ...
        $display("[%0t] <<< CONCURRENT done (enq landed %0d cells)", $time, ei);
