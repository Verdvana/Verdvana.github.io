...
module lle #(
    ...
    assign mc_take_deq   = is_carry_deq & (mc_ncell_q != '0) & (mc_pend_uni_q[deq_port] == '0);
    ...
    logic mc_all_freed;
    always_comb begin
        mc_all_freed = mc_valid_q & (mc_ncell_q != '0);
    ...
    always_comb begin
        ...
        if (enq_grant && lle_set_pkt_tail) begin
            if (enq_is_uni) begin
                ...
                for (int port_idx = 0; port_idx < PORT_NUM; port_idx++) begin : MC_PKT_INC_PORT
                    if (lle_alloc_mcast_bitmap[port_idx])
                        pkt_backlog_inc[carry_qid_c[port_idx]] = 1'b1;
                end
            end
        end
        ...
        else if (deq_grant && mc_take_deq && ((mc_rd_idx_q[deq_port] + 1'b1) == mc_ncell_q)) begin
            pkt_backlog_dec[mc_carry_qid_q[deq_port]] = 1'b1;
        end
    end
                ...
                if (agf_remain_q == 1) begin
                    ...
                    if (agf_qid_q == MC_QID[QID_W-1:0]) begin
                        ...
                        for (int port_idx = 0; port_idx < PORT_NUM; port_idx++) begin : MC_PORT_AGF_CLR
                            if (mc_dst_bitmap_q[port_idx] && !mc_rd_done_q[port_idx] &&
                                (mc_ncell_q != '0) &&
                                (q_uni_pkt_backlog_q[mc_carry_qid_q[port_idx]] != '0))
                                q_uni_pkt_backlog_q[mc_carry_qid_q[port_idx]] <=
                                    q_uni_pkt_backlog_q[mc_carry_qid_q[port_idx]] - 1'b1;
                            mc_rd_done_q[port_idx]  <= 1'b0;
                            mc_rcy_done_q[port_idx] <= 1'b0;
                            mc_pend_uni_q[port_idx] <= '0;
                            mc_rd_idx_q[port_idx]   <= '0;
    ...
    assign lle_q_empty        = enq_pkt_lock_q ? 1'b1 :
    ...
    always_comb begin
        for (int qq = 0; qq < PORT_NUM*TC_NUM; qq++) begin : Q_EMPTY_GEN
            ...
            automatic logic mc_here = mc_valid_q & (mc_ncell_q != '0) &
                                      mc_dst_bitmap_q[pq] &
                                      ~mc_rd_done_q[pq] &
