...
> 建议成片：25 页正文 + 3 页附录，16:9，约 29～33 分钟，适合 RTL 设计交付汇报与架构评审。  
...
1. 第 1～5 页：设计目标、交付规模、模块架构与系统接口。
2. 第 6～9 页：链表与共享池结构、架构取舍和初始化。
3. 第 10～17 页：挂链、走链、还链和零复制多播。
4. 第 18～21 页：仲裁、占用、流控与老化。
5. 第 22～25 页：完整事务、风险、验证与汇报总结。
...
| 内容 | 首选工具 | 交付格式 | 原因 |
|---|---|---|---|
| 模块架构原理图 | ImageGen + PowerPoint 裁切 | 2× PNG | 形成专业、统一的汇报级系统架构视觉 |
| 状态图 | Mermaid Live Editor 或 `mmdc` | SVG | 文本可版本管理，缩放不糊 |
...
```text
project/cores/smmu/design/doc/assets/
├── smmu_module_architecture.png
└── sram_linked_list_structure.png
...
```text
Smart MMU RTL 设计实现汇报
基于系统需求的共享 SRAM 地址管理架构与 RTL 实现

4 Ports × 8 TCs · 8192 Cells · 1R1W 后继指针 SRAM
Architecture · RTL Implementation · Timing · Verification
```
...
> 本次汇报展示依据系统需求完成的 SMMU 架构设计与 RTL 实现，重点说明需求如何落实为可综合的模块划分、链表数据路径、关键时序和验证边界。设计对象是 QM 下方的共享 SRAM 地址管理器，负责 cell 分配、取址、回收、多播引用和老化。

---

## 第 2 页｜设计目标与 RTL 交付结果
...
```text
需求输入：统一管理 QM 的 cell 分配、队列组织、出队取址与地址回收
设计方案：共享 cell 池 + 链表队列 + 集中式链表引擎
RTL 交付：一拍分配 / 一拍队头返回 / 零复制多播 / 占用流控 / 老化回收

交付能力
• 资源效率：32 条调度队列动态共享 8192 个 cell
• 性能目标：无冲突常见路径支持 1 cell/cycle
• 多播效率：一份物理数据、多个端口独立读取
• 生命周期闭环：分配、挂链、取址、回收和重新可用均有明确状态
```

**画面：** 左中右三栏，分别为“需求输入”“RTL 架构”“交付能力”。中间 SMMU 用芯片形状，内部只放四个词：“链表引擎 / 占用管理 / 多播 / 老化”。

**讲稿：** 这一页按“需求—方案—交付”汇报，不采用学习笔记式的问题引入。强调一拍外部接口并不表示内部没有 SRAM 延迟，而是通过队头和两级前瞻寄存器把同步读延迟隐藏在后台。
...
## 第 4 页｜模块架构：LLE 是唯一 SRAM 访问者

**目标 / 时间：** 建立模块职责和数据流，1.5 分钟。

**页面文字：** 页面仅保留标题与模块原理图，不再在图旁堆叠模块职责清单。图片内部全部使用英文专业术语。

**主图：**

![SMMU module architecture](assets/smmu_module_architecture.png)

**图内英文标签：** `QM / Scheduler`、`Enqueue Control`、`Dequeue Control`、`Recycle Control`、`Link-List Engine (LLE)`、`1R1W Next-Ptr SRAM`、`Occupancy & Pool Manager`、`Aging Controller`、`Config / Stats / Init`、`MAC / Egress`、`CSR / CPU`。

**制作：** 使用 ImageGen 生成汇报级 16:9 模块原理图，浅灰底、深蓝模块、青/橙/绿事务流、紫色多播辅助色；采用扁平二维工程框图，不使用 3D、芯片照片、装饰图标或渐变。PNG 以 2× 分辨率插入 PPT，裁切至内容边界并锁定纵横比。动画按“外部接口 → 三类控制器 → LLE → SRAM 与策略模块”四步淡入；不得在 PPT 中覆盖或翻译图片内部英文。
...
## 第 6 页｜链表与共享池结构：逻辑队列如何映射到 SRAM

**目标 / 时间：** 在讨论架构取舍前，先建立“数据 cell、后继指针 SRAM、逻辑队列和统一空闲池”之间的结构关系，1.5 分钟。

**页面文字：**

```text
共享池：所有物理 cell 由统一地址空间管理，未占用 cell 组成空闲链
逻辑队列：每条队列只保存队头、队尾和数量，不要求物理地址连续
后继关系：Next-Ptr SRAM 以 cell 地址为索引，保存下一个 cell 地址及包边界属性
动态迁移：分配从空闲链头取 cell，回收把 cell 重新追加到空闲链尾
```

**主图：**

![Linked-list structure in SRAM](assets/sram_linked_list_structure.png)

**图内英文标签：** `Shared Cell Pool`、`Logical Queue A`、`Logical Queue B`、`Free List`、`Head`、`Tail`、`Cell Address`、`Next-Ptr SRAM`、`Next Address`、`Packet Head`、`Packet Tail`、`Allocate`、`Recycle`。cell 地址示例只使用 `2`、`17`、`5`、`11`、`8`、`21` 等离散整数，体现逻辑连续与物理离散。

**画面：** 左侧约 55% 展示 `Shared Cell Pool`：用规则排列的矩形 cell 表示共享物理地址空间，并以青色、橙色、绿色高亮分别属于 `Logical Queue A`、`Logical Queue B` 和 `Free List` 的离散 cell。用细箭头连接同一逻辑链上的 cell，但不改变 cell 在物理池中的位置。右侧约 45% 展示 `Next-Ptr SRAM` 表格，以 `Cell Address` 为索引，列出 `Next Address`、`Packet Head`、`Packet Tail`；使用三条横向彩色引导线把左侧示例 cell 对应到右侧 SRAM entry。底部用 `Allocate` 箭头表示从空闲链转入业务链，用 `Recycle` 箭头表示回到空闲链。

**制作：** 使用 ImageGen，以上一页模块架构图作为视觉参考，保持相同浅灰背景、深蓝描边、圆角矩形、正交细箭头和无渐变的工程信息图风格。PNG 以 2× 分辨率插入 PPT，锁定纵横比；图片内部只保留英文专业词汇。动画按“共享物理池 → 三条逻辑链 → Next-Ptr SRAM 表项 → Allocate/Recycle”四步淡入。

**讲稿：**

> 数据本体存放在共享 data SRAM 中，链表关系则单独存放在 Next-Ptr SRAM 中。逻辑队列只需要队头、队尾和数量，链上的 cell 地址可以完全离散。所谓共享池，就是所有队列和空闲链共同使用同一批物理 cell；分配与回收改变的是 cell 的链表归属，而不是搬运 payload。

**证据：** `lle.sv` 的队头/队尾状态、空闲链状态和 Next-Ptr SRAM 访问路径；`smmu.sv` 的 SRAM 实例连接。
...
**画面：** 左右两栏必须使用完全一致的“横向队列容量条”视觉语法、相同外框尺寸和相同比例尺。左侧“固定分区”放 Q0/Q1/Q2 三条等长分区条：Q0 占用 92% 并在末端显示红色溢出，Q1 占用 18%，Q2 占用 27%，空白容量只能留在各自队列内。右侧“链表 + 共享池”仍放 Q0/Q1/Q2 三条矩形容量条，不画圆球或串珠链；用统一浅灰共享容量底板承托三条按需伸缩的占用条，Q0 可借用空闲容量吸收突发，Q1/Q2 保持较短占用，底部显示剩余 `Shared Free Space`。底部用一条平衡尺写“利用率 ↑ / 控制复杂度 ↑”。

**制作：** 全部使用 PowerPoint 原生圆角矩形、矩形填充和细线标尺。左右两栏的队列行高、间距、标签位置、颜色和占用百分比标注方式保持一致；差异只通过“固定边界”与“共享底板/动态伸缩”表达。右图禁止使用圆形 cell、穿链箭头或离散地址串珠，以免视觉语言跳变。
...
1. 先建立母版、颜色和页脚，再制作第 4、6、8、13、16、19 页六张核心图。
