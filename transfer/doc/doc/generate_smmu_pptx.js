...
pptx.subject = 'Smart MMU RTL design implementation review';
pptx.title = 'Smart MMU RTL 设计实现汇报';
pptx.lang = 'zh-CN';
pptx.theme = {
...
pptx.defineSlideMaster({
  ...
  objects: [
    ...
    { text: { text: 'Smart MMU RTL Implementation Review', options: { x: 0.56, y: 7.17, w: 3.8, h: 0.2, fontFace: 'Microsoft YaHei', fontSize: 8, color: '7A8798', margin: 0 } } }
  ],
...
// 1. Cover
{
  ...
  addText(s, 'Smart MMU RTL 设计实现汇报', 0.72, 0.8, 8.7, 0.72, { fontSize: 36, bold: true, color: C.white, margin: 0 });
  addText(s, '基于系统需求的共享 SRAM 地址管理架构与 RTL 实现', 0.75, 1.6, 8.9, 0.48, { fontSize: 20, color: 'CFE5F3', margin: 0 });
  ...
  addText(s, 'Architecture · RTL Implementation · Timing · Verification', 0.75, 6.55, 6.2, 0.32, { fontSize: 13, bold: true, color: C.teal, margin: 0 });
  if (typeof s.addNotes === 'function') s.addNotes('本次汇报展示依据系统需求完成的 SMMU 架构设计与 RTL 实现，重点说明模块划分、链表数据路径、关键时序和验证边界。');
}

// 2. Design goals and delivered RTL
{
  const s = newSlide('设计目标与 RTL 交付结果', '定位', '', '按“需求—方案—交付”汇报；一拍接口依赖队头和两级前瞻寄存器隐藏同步 SRAM 读延迟。');
  const cols = [0.72, 4.55, 8.58];
  [['需求输入', C.teal, ['cell 分配', '队列组织', '出队取址与回收']], ['RTL 架构', C.navy, ['共享 cell 池', '链表队列', '集中式链表引擎']], ['交付能力', C.orange, ['一拍分配/队头返回', '占用流控与老化', '零复制多播']]].forEach((g, i) => {
  ...
  const caps = [['资源效率', '32 条队列动态共享 8192 cells', C.teal], ['性能目标', '常见路径支持 1 cell/cycle', C.orange], ['多播效率', '一份数据，多端口独立读取', C.purple], ['生命周期闭环', '分配→挂链→取址→回收', C.green]];
...
// 4. Architecture
{
  const s = newSlide('模块架构：LLE 是唯一 SRAM 访问者', '定位', '', '控制器负责命令与结果流水；LLE 集中维护链表权威状态并独占 Next-Ptr SRAM 接口。');
  s.addImage({ path: path.join(__dirname, 'assets', 'smmu_module_architecture.png'), x: 1.42, y: 0.9, w: 10.49, h: 5.9 });
}
...
// 6. Linked-list structure in SRAM
{
  const s = newSlide('链表与共享池结构：逻辑队列如何映射到 SRAM', '数据结构', '', '数据 cell 位于共享数据 SRAM，链表关系位于 Next-Ptr SRAM；分配和回收改变 cell 的链表归属，不搬运 payload。');
  s.addImage({ path: path.join(__dirname, 'assets', 'sram_linked_list_structure.png'), x: 1.42, y: 0.9, w: 10.49, h: 5.9 });
}

// 7. Why linked list
{
  const s = newSlide('为什么选择链表 + 共享池', '数据结构', '', '左右两侧使用相同的横向容量条视觉语言；固定分区不能借用空闲容量，共享池可按需伸缩。');
  ...
  used.forEach((u, i) => {
    addText(s, `Q${i}`, 0.88, 2.16 + i * 0.82, 0.38, 0.43, { fontSize: 12, bold: true, color: C.navy, align: 'center', margin: 0 });
    box(s, 1.35, 2.15 + i * 0.82, 4.35, 0.48, '', C.white, C.gray3, { shape: S.rect });
    s.addShape(S.rect, { x: 1.35, y: 2.15 + i * 0.82, w: 4.35 * u, h: 0.48, fill: { color: i === 0 ? C.red : C.gray2 }, line: { color: i === 0 ? C.red : C.gray2 } });
    addText(s, `${Math.round(u * 100)}%`, i === 0 ? 4.48 : 4.95, 2.16 + i * 0.82, 0.62, 0.4, { fontSize: 11, bold: true, color: i === 0 ? C.white : C.navy, align: 'right', margin: 0 });
  });
  pill(s, 4.98, 2.72, 0.82, '溢出', C.red);
  ...
  const shared = [0.92, 0.18, 0.27];
  const sharedColors = [C.teal, C.orange, C.purple];
  shared.forEach((u, i) => {
    addText(s, `Q${i}`, 7.33, 2.16 + i * 0.82, 0.38, 0.43, { fontSize: 12, bold: true, color: C.navy, align: 'center', margin: 0 });
    box(s, 7.8, 2.15 + i * 0.82, 4.35, 0.48, '', 'EDF2F7', C.gray3, { shape: S.rect });
    s.addShape(S.rect, { x: 7.8, y: 2.15 + i * 0.82, w: 4.35 * u, h: 0.48, fill: { color: sharedColors[i] }, line: { color: sharedColors[i] } });
    addText(s, `${Math.round(u * 100)}%`, i === 0 ? 10.93 : 11.4, 2.16 + i * 0.82, 0.62, 0.4, { fontSize: 11, bold: true, color: i === 0 ? C.white : C.navy, align: 'right', margin: 0 });
  });
  box(s, 7.8, 4.55, 4.35, 0.42, 'Shared Free Space', C.white, C.green, { shape: S.rect, text: { fontSize: 12, bold: true, color: '15803D', align: 'center', margin: 0 } });
  addText(s, '热点队列借用共享余量，突发被池化吸收', 7.55, 5.15, 4.65, 0.42, { fontSize: 16, bold: true, color: '15803D', align: 'center' });
}
