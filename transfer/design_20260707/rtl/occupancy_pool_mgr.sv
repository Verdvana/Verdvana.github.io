    ...
    always_ff @(posedge clk_core or negedge rst_core_n) begin
        if (!rst_core_n) begin
            ...
            for (int q_idx = 0; q_idx < QUEUE_NUM; q_idx++) begin : Q_CELL_CNT_UPDATE
                if (q_cell_inc[q_idx] && !q_cell_dec[q_idx]) begin
                    q_cell_cnt_q[q_idx] <= q_cell_cnt_q[q_idx] + 1'b1;
                end
                else if (!q_cell_inc[q_idx] && q_cell_dec[q_idx]) begin
                    q_cell_cnt_q[q_idx] <= q_cell_cnt_q[q_idx] - 1'b1;
                end
            end
        end
    ...
    always_ff @(posedge clk_core or negedge rst_core_n) begin
        if (!rst_core_n) begin
            ...
            for (int q_idx = 0; q_idx < QUEUE_NUM; q_idx++) begin : Q_STATIC_USED_UPDATE
                if (q_static_inc[q_idx] && !q_static_dec[q_idx]) begin
                    q_static_used_q[q_idx] <= q_static_used_q[q_idx] + 1'b1;
                end
                else if (!q_static_inc[q_idx] && q_static_dec[q_idx]) begin
                    q_static_used_q[q_idx] <= q_static_used_q[q_idx] - 1'b1;
                end
            end
        end
    ...
    always_ff @(posedge clk_core or negedge rst_core_n) begin
        if (!rst_core_n) begin
            ...
            for (int port_idx = 0; port_idx < PORT_NUM; port_idx++) begin : PER_PORT_USED_UPDATE
                if (port_inc[port_idx] && !port_dec[port_idx]) begin
                    per_port_used_q[port_idx] <= per_port_used_q[port_idx] + 1'b1;
                end
                else if (!port_inc[port_idx] && port_dec[port_idx]) begin
                    per_port_used_q[port_idx] <= per_port_used_q[port_idx] - 1'b1;
                end
            end
        end
