...
module enqueue_ctrl #(
    ...
    localparam int TC_W     = (TC_NUM    > 1) ? $clog2(TC_NUM)    : 1 // enq_queue_id 位宽 (仅 TC)
)(
    //------------------------------------------------------------------------
    // 时钟复位 / 初始化 (公共)
    //------------------------------------------------------------------------
    ...
    input  logic                  mc_busy,             // ★ B2: 所有多播槽均忙, 置1时新多播整帧丢弃
    ...
    output logic [TC_W-1:0]       lle_alloc_mcast_tc // ★ B2: 组播帧 TC → LLE (定承载队列)
);
    ...
    // 使用乘加而非拼接/移位，TC_NUM 非 2 的幂时仍保持 QID 连续。
    assign uni_qid_c  = QID_W'(enq_egress_port) * QID_W'(TC_NUM) +
                        QID_W'(enq_queue_id);
    assign full_qid_c = enq_is_mcast ? MC_QID[QID_W-1:0] : uni_qid_c;

    //========================================================================
    // 占用判决查询 (组合, 当拍返回): 透传 vld + 当前队列/端口给 Occupancy,
    //   occ_accept/occ_drop/occ_no_free 组合返回。
    //========================================================================
    assign occ_query_vld         = enq_fire;
    assign occ_query_queue_id    = full_qid_c;           // ★ 完整队列号 (单播=port*TC_NUM+tc; 多播=MC_QID)
    ...
    //========================================================================
    // ★ B2 多槽门控: 多播帧到达 (SOF) 时若所有帧槽已占用 → 整帧丢弃。
    //   mc_busy 由 LLE 的槽空闲检测提供, T0 当拍可读。
    ...
    assign lle_alloc_queue_id     = full_qid_c;          // ★ 完整队列号 (单播=port*TC_NUM+tc; 多播=MC_QID)
