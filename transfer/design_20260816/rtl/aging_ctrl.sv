...
module aging_ctrl #(
    ...
    always_ff @(posedge clk_core or negedge rst_core_n) begin
        if (!rst_core_n) begin
            for (int i = 0; i < QUEUE_NUM; i++) begin : AGE_TIMER_RST_Q
                age_timer_q[i] <= '0;
                age_trig_q[i]  <= 1'b0;
            end
        end
        else if (clr_ptr_cnt) begin              // ★ 初始化期同步清
            for (int i = 0; i < QUEUE_NUM; i++) begin : AGE_TIMER_CLR_Q
                age_timer_q[i] <= '0;
                age_trig_q[i]  <= 1'b0;
            end
        end
        else begin
            for (int i = 0; i < QUEUE_NUM; i++) begin : AGE_TIMER_UPDATE_Q
                ...
                // 计时递增
                    ...
                    age_timer_q[i] <= age_timer_q[i] + 1'b1;
                end
            end
        end
    end
    ...
    always_comb begin
        for (int q_idx = 0; q_idx < QUEUE_NUM; q_idx++) begin : AGE_TRIG_Q
            age_trig[q_idx] = (cfg_aging_en && init_done) &
                              (age_trig_q[q_idx] | cfg_age_force[q_idx]);
        end
    end
    ...
    always_comb begin
        found     = 1'b0;
        found_qid = '0;
        for (int k = 0; k < QUEUE_NUM; k++) begin : AGE_RR_SEARCH
    ...
    //========================================================================
    always_comb begin
        for (int port_idx = 0; port_idx < PORT_NUM; port_idx++) begin : PORT_AGE_AGG_PORT
            automatic logic acc = 1'b0;
            for (int tc_idx = 0; tc_idx < TC_NUM; tc_idx++) begin : PORT_AGE_AGG_TC
                acc = acc | age_trig[port_idx*TC_NUM + tc_idx];
            end
            port_age_trig[port_idx] = acc;
        end
    end
