...
module csr_stats_init #(
    ...
    always_ff @(posedge clk_core or negedge rst_core_n) begin
        if (!rst_core_n) init_start_q <= 2'b00;
        else             init_start_q <= {init_start_q[0], init_start};
    end
    ...
    always_ff @(posedge clk_core or negedge rst_core_n) begin
        if (!rst_core_n) begin
            ...
            cfg_q_min_cell        <= cfg_in_q_min_cell;
            cfg_q_max_cell        <= cfg_in_q_max_cell;
            cfg_port_max          <= cfg_in_port_max;
            cfg_global_max        <= cfg_in_global_max;
            ...
            cfg_aging_en          <= cfg_in_aging_en;
            cfg_aging_timeout     <= cfg_in_aging_timeout;
            cfg_age_force         <= cfg_in_age_force;
        end
    end
    ...
    always_ff @(posedge clk_core or negedge rst_core_n) begin
        if (!rst_core_n) begin
            ...
            st_out_q_max_reached_status   <= '0;
            st_out_tail_drop_cnt          <= '0;
            st_out_q_max_assert_cnt       <= '0;
            ...
            st_out_q_max_reached_status   <= st_q_max_reached_status;
            st_out_tail_drop_cnt          <= st_tail_drop_cnt;
            st_out_q_max_assert_cnt       <= st_q_max_assert_cnt;
    ...
    always_ff @(posedge clk_core or negedge rst_core_n) begin
        if (!rst_core_n) begin
            ...
            irq_aging <= aging_irq_in;   // ★ 来自 aging_ctrl (有队列被老化即置)
        end
    end
    ...
    always_ff @(posedge clk_core or negedge rst_core_n) begin
        if (!rst_core_n) state_curr <= IS_IDLE;
