...
module dequeue_ctrl #(
    ...
    input  logic [TC_W-1:0]       deq_queue_id,        // 出队 TC/queue id
    ...
    input  logic                  lle_qhead_next_pkt_tail, // 队头下一 cell 尾标志
    ...
    //========================================================================
    // 出队端口/TC → 内部完整队列号: full_qid = port*TC_NUM + tc。
    //========================================================================
    logic [QID_W-1:0]  full_deq_qid;
    logic [PORT_W-1:0] egress_port_idx;
    assign full_deq_qid    = QID_W'(deq_egress_port) * QID_W'(TC_NUM) +
                             QID_W'(deq_queue_id);
    // PORT_NUM 是“数量”而非合法 ID；不能压成 PORT_W 位，否则 2 的幂配置会截断为 0。
    assign egress_port_idx = (deq_egress_port < PORT_NUM) ? deq_egress_port : '0;
    ...
    assign lle_deq_queue_id = full_deq_qid;
    ...
    always_ff @(posedge clk_core or negedge rst_core_n) begin
        if (!rst_core_n) begin
            ...
            deq_pkt_tail_next <= deq_fire & lle_qhead_next_pkt_tail & ~lle_qhead_pkt_tail;
        end
    end
