...
module recycle_ctrl #(
    ...
    input  logic [TC_W-1:0]       recycle_queue_id,    // 单播回收 cell 所属 TC/queue id (多播命中时忽略)
    input  logic [PORT_W-1:0]     recycle_egress_port, // 单播回收 cell 所属端口 ID (多播命中时忽略)
    ...
    //========================================================================
    logic [QID_W-1:0] full_recycle_qid;

    assign full_recycle_qid = QID_W'(recycle_egress_port) * QID_W'(TC_NUM) +
                              QID_W'(recycle_queue_id);

    assign lle_free_req      = recycle_req;
    assign lle_free_addr     = recycle_cell_addr;
    assign lle_free_queue_id = full_recycle_qid;
