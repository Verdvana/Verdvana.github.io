//============================================================================
// Module      : lle  (Link-List Engine) —— B2 多播逻辑拼接版
// Project     : 4-Port 2.5/1G/100M Ethernet Switch - Smart MMU
//
// Description :
//   LLE 是 MMU 的存储访问平面核心: 唯一访问 Next-Ptr SRAM (1R1W), 对 Enqueue /
//   Dequeue / Recycle 提供分配/出队/还链服务。
//
//   ── 链表组织 ──
//     [0..31]  : per-(port,TC) 单播队列链
//     [32]     : MC_QID 只是多播占用统计 ID。每个多播帧槽保存自己的
//                mc_cells_q[slot][] 地址列表，并在 Next-Ptr SRAM 中链成独立帧链，
//                以便老化时将完整帧链一次拼接到 free 链。
//     free 链  : 独立维护 (free_head/free_tail/free_cnt 寄存器)
//
//   ── B2 多播模型 (2~4 槽 + 零复制 + 逐端口私有读指针) ──
//     1) MAX_MC_FRAMES 个小型寄存器槽支持多帧并发; 全部占用时 mc_busy=1。
//     2) 每帧数据只存一份; mc_cells_q[slot][] 保存该帧 cell 地址序列。
//     3) 每槽、每目的端口有独立读索引; pend 计数保证同一承载队列帧顺序。
    //     4) 逻辑插入位置: 多播帧 SOF 到达时对每个目的端口快照“承载单播队列”当时在队
    //        的完整包 backlog mc_pend_uni_q[p]; 出队每出完一个前序单播包(pkt_tail)-1;
//        减到 0 且多播未读完 → 该端口下一个包切多播 cell-list。
//     5) 回收: EPS 按目的端口逐 cell 回收; 每 cell 引用计数归零时才还 free。
//        某帧所有 cell 引用均归零后只清该槽，其他多播帧可继续并发。
//     6) 双池按实际存入计: 多播 alloc/free 事件均用 MC_QID, global 只 ±M 一次。
//
// Clock/Reset : clk_core (300MHz, 单时钟域) / rst_core_n (异步复位低有效)
//============================================================================
`timescale 1ns/1ps

module lle #(
    parameter  int CELL_NUM        = 8192,
    parameter  int PORT_NUM        = 4,
    parameter  int TC_NUM          = 8,      // 每端口 TC 数
    parameter  int REF_W           = 3,      // (兼容保留, 未用)
    parameter  int RCY_FIFO_DEPTH  = 8,
    parameter  int MAX_MC_CELLS    = 8,      // 多播帧最大 cell 数 (1522B/256B=6, 取 8 余量)
    parameter  int MAX_MC_FRAMES   = 4,      // 并发多播帧槽数, 支持 2~4
    // ★ 队列数 = 端口数×每端口TC + 1 (1 多播专用队列; free 链独立)
    localparam int QUEUE_NUM       = PORT_NUM*TC_NUM + 1,
    localparam int MC_QID          = QUEUE_NUM-1,        // 多播队列号 (=32)
    localparam int ADDR_W          = $clog2(CELL_NUM),
    localparam int QID_W           = (QUEUE_NUM > 1) ? $clog2(QUEUE_NUM) : 1,
    localparam int PORT_W          = (PORT_NUM  > 1) ? $clog2(PORT_NUM)  : 1,
    localparam int TC_W            = (TC_NUM    > 1) ? $clog2(TC_NUM)    : 1,
    localparam int CNT_W           = ADDR_W + 1,
    localparam int ENTRY_W         = ADDR_W + 2,         // entry = {next, ph, pt}
    localparam int PH_BIT          = 1,
    localparam int PT_BIT          = 0,
    localparam int MC_IDX_W        = $clog2(MAX_MC_CELLS+1),
    localparam int MC_SLOT_W       = (MAX_MC_FRAMES > 1) ? $clog2(MAX_MC_FRAMES) : 1
)(
    input  logic                  clk_core,
    input  logic                  rst_core_n,

    // Init FSM
    input  logic                  init_build_req,
    output logic                  init_build_done,

    // Enqueue Ctrl
    output logic [ADDR_W-1:0]     lle_free_head,
    output logic                  lle_free_empty,
    output logic                  lle_alloc_ready,
    input  logic                  lle_alloc_fire,
    input  logic [QID_W-1:0]      lle_alloc_queue_id,
    input  logic                  lle_set_pkt_head,
    input  logic                  lle_set_pkt_tail,
    input  logic                  lle_alloc_is_mcast,
    input  logic [PORT_NUM-1:0]   lle_alloc_mcast_bitmap,
    input  logic [TC_W-1:0]       lle_alloc_mcast_tc,   // ★ 多播帧 TC (决定各端口承载队列)
    output logic                  mc_busy,

    // Dequeue Ctrl (含多播 splice)
    input  logic [QID_W-1:0]      lle_deq_queue_id,
    output logic [ADDR_W-1:0]     lle_qhead,
    output logic                  lle_qhead_pkt_head,
    output logic                  lle_qhead_pkt_tail,
    output logic                  lle_qhead_next_pkt_tail,
    output logic                  lle_q_empty,
    input  logic                  lle_deq_fire,

    // ★ B2: 给 QM 的 32 条常规队列 empty 向量 (多播计入各目的端口承载队列)
    output logic [PORT_NUM*TC_NUM-1:0] q_empty_vec,

    // ★ 给 QM 的 32 条常规队列 "pkt 数为 0" 向量 (完整包粒度; 多播计入各目的端口承载队列)
    //   与 q_empty_vec 位宽/索引一致, 但以 "在队完整包数" 而非 cell 数判空。
    output logic [PORT_NUM*TC_NUM-1:0] q_pkt_empty_vec,

    // Recycle Ctrl (统一还链: 单播直接还; 多播按 cell ref-count, 减到 0 才还)
    input  logic                  lle_free_req,
    input  logic [ADDR_W-1:0]     lle_free_addr,
    input  logic [QID_W-1:0]      lle_free_queue_id,
    input  logic                  lle_free_is_mcast,  // 该 cell 是否多播 (提示位)
    output logic                  lle_free_grant,
    output logic                  lle_free_done,
    output logic                  mcast_underflow,    // 多播还链下溢 (对已还完/未命中的多播 cell 又还)

    // ★ 老化冲刷 (aging_ctrl ↔ LLE): 把某队列整链一次性拼接到 free 链
    input  logic                  age_flush_req,      // 请求冲刷某队列
    input  logic [QID_W-1:0]      age_flush_qid,      // 待冲刷队列号
    output logic                  age_flush_busy,     // 正在冲刷
    output logic                  age_flush_done,     // 冲刷完成 (队列已空)
    // ★ 老化用: 队列非空位图 + 出队 fire 喂狗信号 (→ aging_ctrl)
    //   位宽 QUEUE_NUM (=32 单播 + 1 多播), 高位 MC_QID 位表示多播槽占用
    output logic [QUEUE_NUM-1:0]  q_occupied_vec,
    output logic                  deq_fire_evt,
    output logic [QID_W-1:0]      deq_fire_qid,

    // Occupancy
    output logic                  lle_alloc_evt,
    output logic [QID_W-1:0]      evt_queue_id,
    output logic [PORT_W-1:0]     evt_egress_port,
    output logic                  lle_free_evt,
    output logic [QID_W-1:0]      evt_free_queue_id,
    output logic [PORT_W-1:0]     evt_free_egress_port,
    output logic [CNT_W-1:0]      evt_free_cell_num,
    // 低功耗时钟控制: LLE 内部仍有必须排空的操作
    output logic                  lle_busy
);

    //========================================================================
    // per-queue 链表寄存器 (两级预取, 仅单播队列使用; MC_QID 不走 SRAM)
    //========================================================================
    logic [ADDR_W-1:0]   q_head_q       [QUEUE_NUM];
    logic [ADDR_W-1:0]   q_tail_q       [QUEUE_NUM];
    logic [CNT_W-1:0]    q_cell_cnt_q   [QUEUE_NUM];
    logic                q_head_ph_q    [QUEUE_NUM];
    logic                q_head_pt_q    [QUEUE_NUM];
    logic [ADDR_W-1:0]   q_head_next_q    [QUEUE_NUM];
    logic                q_head_next_ph_q [QUEUE_NUM];
    logic                q_head_next_pt_q [QUEUE_NUM];
    logic [ADDR_W-1:0]   q_head_next2_q   [QUEUE_NUM];
    logic                q_tail_ph_q      [QUEUE_NUM];
    logic                q_tail_pt_q      [QUEUE_NUM];

    // ★ 每常规调度队列: 在队完整包计数
    //   单播 EOF 后对其队列 +1; 多播 EOF 后对 bitmap 命中的各承载队列 +1。
    //   出队到 pkt_tail 后立即 -1。
    logic [CNT_W-1:0]    q_uni_pkt_backlog_q [QUEUE_NUM];

    //========================================================================
    // free 链寄存器 (两级预取)
    //========================================================================
    logic [ADDR_W-1:0]   free_head_q;
    logic [ADDR_W-1:0]   free_tail_q;
    logic [CNT_W-1:0]    free_cnt_q;
    logic [ADDR_W-1:0]   free_head_next_q;
    logic [ADDR_W-1:0]   free_head_next2_q;

    assign lle_free_head  = free_head_q;
    assign lle_free_empty = (free_cnt_q == '0);

    //========================================================================
    // ★ B2 多播槽 (2~4 槽, 全部为小型寄存器阵列)
    //========================================================================
    logic [MAX_MC_FRAMES-1:0] mc_valid_q;
    logic [PORT_NUM-1:0]      mc_dst_bitmap_q [MAX_MC_FRAMES];
    logic [QID_W-1:0]         mc_carry_qid_q  [MAX_MC_FRAMES][PORT_NUM];
    logic [ADDR_W-1:0]        mc_cells_q      [MAX_MC_FRAMES][MAX_MC_CELLS];
    logic [MC_IDX_W-1:0]      mc_ncell_q      [MAX_MC_FRAMES];
    logic [MC_IDX_W-1:0]      mc_wr_idx_q     [MAX_MC_FRAMES];
    logic [MC_IDX_W-1:0]      mc_rd_idx_q     [MAX_MC_FRAMES][PORT_NUM];
    logic                     mc_rd_done_q    [MAX_MC_FRAMES][PORT_NUM];
    logic [CNT_W-1:0]         mc_pend_uni_q   [MAX_MC_FRAMES][PORT_NUM];
    logic [ADDR_W-1:0]        mc_head_q       [MAX_MC_FRAMES];
    logic [ADDR_W-1:0]        mc_tail_q       [MAX_MC_FRAMES];
    logic [MC_SLOT_W-1:0]     mc_build_slot_q;

    // ★ 统一还链: 每槽每 cell 引用计数。
    //   mc_ref_cnt_q[slot][i] = 该帧第 i 个 cell 尚待还链的目的端口数。
    //   QM 逐 cell 还链, 命中某 slot → 该 slot cnt--; 减到 0 → 真正 push 还回 free 链。
    localparam int MC_REF_W = $clog2(PORT_NUM+1);   // N 最大 = PORT_NUM
    logic [MC_REF_W-1:0]  mc_ref_cnt_q [MAX_MC_FRAMES][MAX_MC_CELLS];
    logic [MC_REF_W-1:0]  mc_dst_cnt_q [MAX_MC_FRAMES];
    logic [MC_REF_W-1:0]  mc_dst_cnt_c;             // 本帧目的端口数 = popcount(bitmap)

    logic                    mc_free_slot_vld;
    logic [MC_SLOT_W-1:0]    mc_free_slot_c;
    logic [MC_SLOT_W-1:0]    mc_enq_slot_c;

    always_comb begin
        mc_free_slot_vld = 1'b0;
        mc_free_slot_c   = '0;
        for (int slot_idx = 0; slot_idx < MAX_MC_FRAMES; slot_idx++) begin
            if (!mc_valid_q[slot_idx] && !mc_free_slot_vld) begin
                mc_free_slot_vld = 1'b1;
                mc_free_slot_c   = MC_SLOT_W'(slot_idx);
            end
        end
    end

    assign mc_enq_slot_c = lle_set_pkt_head ? mc_free_slot_c : mc_build_slot_q;
    assign mc_busy       = ~mc_free_slot_vld;

    // popcount(mcast_bitmap) → 本帧目的端口数
    always_comb begin
        mc_dst_cnt_c = '0;
        for (int p = 0; p < PORT_NUM; p++)
            mc_dst_cnt_c += MC_REF_W'(lle_alloc_mcast_bitmap[p]);
    end

    // 各端口承载单播队列号 (组合): carry_qid[p] = p*TC_NUM + 多播帧TC
    //   ★ 多播帧只有一个 TC/优先级, 在每个目的端口都落到该 TC 的队列上 → 与 QM 调度一致。
    //     (QM 出队某端口的 该TC队列 → MMU 在此队列上 splice 出多播报文)
    logic [QID_W-1:0] carry_qid_c [PORT_NUM];

    always_comb begin
        for (int port_idx = 0; port_idx < PORT_NUM; port_idx++) begin : CARRY_QID
            carry_qid_c[port_idx] = QID_W'(port_idx*TC_NUM) + QID_W'(lle_alloc_mcast_tc);
        end
    end

    //========================================================================
    // Recycle FIFO (还链 cell 缓冲)
    //========================================================================
    logic [ADDR_W-1:0]   rcy_fifo_mem [RCY_FIFO_DEPTH];
    logic [$clog2(RCY_FIFO_DEPTH+1)-1:0] rcy_fifo_cnt_q;
    logic [$clog2(RCY_FIFO_DEPTH)-1:0]   rcy_fifo_wptr_q, rcy_fifo_rptr_q;
    logic                rcy_fifo_full, rcy_fifo_empty;
    assign rcy_fifo_full  = (rcy_fifo_cnt_q == RCY_FIFO_DEPTH);
    assign rcy_fifo_empty = (rcy_fifo_cnt_q == '0);

    logic [ADDR_W-1:0]   rcy_cell;
    assign rcy_cell = rcy_fifo_mem[rcy_fifo_rptr_q];

    logic do_push, do_pop;

    //========================================================================
    // Next-Ptr SRAM (1R1W) 互连
    //========================================================================
    logic                npr_r_en;
    logic [ADDR_W-1:0]   npr_r_addr;
    logic [ENTRY_W-1:0]  npr_r_data;
    logic                npr_w_en;
    logic [ADDR_W-1:0]   npr_w_addr;
    logic [ENTRY_W-1:0]  npr_w_data;

    //========================================================================
    // 建链 FSM
    //========================================================================
    typedef enum logic [1:0] {ST_IDLE=2'b00, ST_BUILD=2'b01, ST_DONE=2'b10} build_st_e;
    build_st_e          build_st_q;
    logic [ADDR_W-1:0]  build_idx_q;
    logic               build_active;
    assign build_active = (build_st_q == ST_BUILD);

    always_ff @(posedge clk_core or negedge rst_core_n) begin
        if (!rst_core_n) begin
            build_st_q      <= ST_IDLE;
            build_idx_q     <= '0;
            init_build_done <= 1'b0;
        end
        else begin
            unique case (build_st_q)
                ST_IDLE: begin
                    init_build_done <= 1'b0;
                    if (init_build_req) begin
                        build_st_q  <= ST_BUILD;
                        build_idx_q <= '0;
                    end
                end
                ST_BUILD: begin
                    if (build_idx_q == CELL_NUM-1) build_st_q <= ST_DONE;
                    build_idx_q <= build_idx_q + 1'b1;
                end
                ST_DONE: begin
                    init_build_done <= 1'b1;
                    build_st_q      <= ST_IDLE;
                end
                default: begin
                    build_st_q      <= ST_IDLE;
                    build_idx_q     <= '0;
                    init_build_done <= 1'b0;
                end
            endcase
        end
    end

    //========================================================================
    // 多播出队目标端口/承载判定 (对当前 lle_deq_queue_id, 组合)
    //========================================================================
    logic [QID_W-1:0]   deq_port_full;
    logic               deq_is_uni;
    logic [PORT_W-1:0]  deq_port;
    logic               mc_take_deq;
    logic               mc_deq_slot_vld;
    logic [MC_SLOT_W-1:0] mc_deq_slot_c;

    assign deq_port_full = lle_deq_queue_id / TC_NUM;
    assign deq_is_uni    = (lle_deq_queue_id < (PORT_NUM*TC_NUM));
    assign deq_port      = deq_is_uni ? deq_port_full[PORT_W-1:0] : '0;
    always_comb begin
        mc_deq_slot_vld = 1'b0;
        mc_deq_slot_c   = '0;
        if (deq_is_uni) begin
            // 同一承载队列中，只有前序包数已归零的最早槽可出队。
            for (int slot_idx = 0; slot_idx < MAX_MC_FRAMES; slot_idx++) begin
                if (mc_valid_q[slot_idx] && (mc_ncell_q[slot_idx] != '0) &&
                    mc_dst_bitmap_q[slot_idx][deq_port] &&
                    !mc_rd_done_q[slot_idx][deq_port] &&
                    (lle_deq_queue_id == mc_carry_qid_q[slot_idx][deq_port])) begin
                    if ((mc_pend_uni_q[slot_idx][deq_port] == '0) && !mc_deq_slot_vld) begin
                        mc_deq_slot_vld = 1'b1;
                        mc_deq_slot_c   = MC_SLOT_W'(slot_idx);
                    end
                end
            end
        end
    end
    assign mc_take_deq = mc_deq_slot_vld;

    //========================================================================
    // 事务请求信号
    //========================================================================
    logic enq_req_int, enq_is_uni;
    logic deq_req_int, deq_need_sram_raw, deq_need_sram;
    logic rcy_req_int;
    logic enq_pkt_lock_q;
    logic agf_active;

    assign enq_is_uni  = ~lle_alloc_is_mcast;
    assign enq_req_int = lle_alloc_fire & ~build_active & ~lle_free_empty & ~agf_active;

    // deq: 多播 take 或单播队列非空
    assign deq_req_int   = lle_deq_fire & ~build_active & ~agf_active &
                           (mc_take_deq | (q_cell_cnt_q[lle_deq_queue_id] != '0));
    // 仅单播走链且 cnt>=3 时需要 SRAM 读; 多播 take 不读 SRAM
    assign deq_need_sram_raw = deq_req_int & ~mc_take_deq &
                               (q_cell_cnt_q[lle_deq_queue_id] >= 3);
    assign deq_need_sram = deq_need_sram_raw & ~enq_pkt_lock_q;
    assign rcy_req_int   = ~rcy_fifo_empty & ~build_active;

    //========================================================================
    // 三选一仲裁:
    //   - 空闲时沿用 deq > enq > rcy。
    //   - 一旦 enq 被 grant 且 packet 未到 EOF, 锁住 enq packet, 直到 EOF cell
    //     被接收才释放; 锁住期间 deq/rcy 不打断该 packet。
    //========================================================================
    logic deq_grant, enq_grant, rcy_grant;
    assign deq_grant = deq_req_int & ~enq_pkt_lock_q;
    assign enq_grant = enq_req_int & ~build_active & ~deq_need_sram;
    assign rcy_grant = rcy_req_int & ~build_active & ~deq_need_sram & ~enq_grant;

    assign lle_alloc_ready = ~build_active & ~lle_free_empty & ~deq_need_sram & ~agf_active;

    always_ff @(posedge clk_core or negedge rst_core_n) begin
        if (!rst_core_n) begin
            enq_pkt_lock_q <= 1'b0;
        end
        else if (build_active) begin
            enq_pkt_lock_q <= 1'b0;
        end
        else if (enq_grant) begin
            enq_pkt_lock_q <= ~lle_set_pkt_tail;
        end
    end

    //========================================================================
    // ★ 老化冲刷整链拼接: 一次把待冲刷队列挂到 free 链尾。
    //   free 非空: old_free_tail.next = aged_head, free_tail = aged_tail。
    //   free 为空: free head/tail 及两级预取寄存器直接接管 aged queue。
    //   同拍批量更新 free/queue/port/global 计数，不再逐 cell walk。
    //========================================================================
    typedef enum logic [1:0] {AGF_IDLE, AGF_SPLICE, AGF_DONE} agf_st_e;
    agf_st_e            agf_st_q;
    logic [QID_W-1:0]   agf_qid_q;        // 正在冲刷的队列
    logic [ADDR_W-1:0]  agf_head_q;       // 待拼接链 head 快照
    logic [ADDR_W-1:0]  agf_tail_q;       // 待拼接链 tail 快照
    logic [ADDR_W-1:0]  agf_head_next_q;  // free 为空时恢复两级预取
    logic [ADDR_W-1:0]  agf_head_next2_q;
    logic [CNT_W-1:0]   agf_cell_num_q;   // 本次批量释放 cell 数
    logic [MC_SLOT_W-1:0] agf_mc_slot_q;
    logic                  mc_age_slot_vld;
    logic [MC_SLOT_W-1:0]  mc_age_slot_c;

    assign agf_active = (agf_st_q != AGF_IDLE);
    assign age_flush_busy = agf_active;

    always_comb begin
        mc_age_slot_vld = 1'b0;
        mc_age_slot_c   = '0;
        for (int slot_idx = 0; slot_idx < MAX_MC_FRAMES; slot_idx++) begin
            if (mc_valid_q[slot_idx] && (mc_ncell_q[slot_idx] != '0) && !mc_age_slot_vld) begin
                mc_age_slot_vld = 1'b1;
                mc_age_slot_c   = MC_SLOT_W'(slot_idx);
            end
        end
    end

    // 整链拼接使用 SRAM 写口，优先级低于正常 enq/recycle。
    logic               agf_splice_gnt;

    //========================================================================
    // 还链 push 源仲裁: 外部单播回收 / 多播引用计数释放。
    //========================================================================
    logic               ext_free_push;   // 外部单播回收 push
    logic               mc_rel_push;      // 多播整帧还链 push
    logic [ADDR_W-1:0]  push_cell;
    logic [QID_W-1:0]   push_qid;


    logic                 mc_rcy_hit;              // 本还链请求命中多播槽某 cell
    logic [MC_SLOT_W-1:0] mc_rcy_hit_slot;         // 命中的帧槽
    logic [MC_IDX_W-1:0]  mc_rcy_hit_idx;          // 命中的 slot 索引
    logic                 mc_rcy_last;             // 命中 slot 递减后归 0 (本 cell 真正还)

    // ext_free_push: 外部还链真正 push 回 free 链的条件:
    //   - 单播 (未命中多播槽): 直接 push, occ 用 lle_free_queue_id;
    //   - 多播命中且是该 cell 最后一次还 (mc_rcy_last): push, occ 用 MC_QID;
    //   - 多播命中但非最后一次 (仅递减 ref, 不 push): ext_free_push=0 (只在 ff 段递减)。
    //   注: mc_rcy_hit/mc_rcy_last 见下方还链命中判定 (组合, 前向引用于此处 assign 之后定义,
    //       但同为组合信号, 综合/仿真无序无碍)。
    logic ext_free_do_free;   // 本还链请求本拍是否真正还回 free (=push)
    assign ext_free_do_free = lle_free_req & (~mc_rcy_hit | mc_rcy_last);
    assign ext_free_push = ext_free_do_free & ~rcy_fifo_full & ~build_active & ~agf_active;
    // 正常多播回收改为逐 cell ref-count，不再走整帧 walk。
    assign mc_rel_push   = 1'b0;

    assign push_cell = ext_free_push ? lle_free_addr : '0;
    // push_qid: 多播命中 → MC_QID (occ 按多播池计); 否则单播 → lle_free_queue_id
    assign push_qid  = ext_free_push ? (mc_rcy_hit ? MC_QID[QID_W-1:0] : lle_free_queue_id) :
                                       MC_QID[QID_W-1:0];

    assign do_push = ext_free_push | mc_rel_push;
    assign do_pop  = rcy_grant;

    //========================================================================
    // SRAM 读写口驱动
    //========================================================================
    logic [ADDR_W-1:0]  build_addr;
    logic [ENTRY_W-1:0] build_wdata;
    assign build_addr  = build_idx_q;
    assign build_wdata = {(build_idx_q == CELL_NUM-1) ? build_idx_q : (build_idx_q + 1'b1),
                          1'b0, 1'b0};

    logic [ADDR_W-1:0] enq_cell;
    assign enq_cell = free_head_q;

    // pend 流水寄存器
    logic               deq_pend_q;
    logic [QID_W-1:0]   deq_pend_qid_q;
    logic               enq_pend_q;
    logic               deq_pend_tail_q;
    logic               deq_pend_tail_ph_q;
    logic               deq_pend_tail_pt_q;

    logic deq_pend_same_q;
    logic enq_bypass;
    assign deq_pend_same_q = deq_pend_q & deq_grant & ~mc_take_deq &
                             (deq_pend_qid_q == lle_deq_queue_id);
    assign enq_bypass      = enq_pend_q & enq_grant;

    logic [ADDR_W-1:0] enq_sram_rd_addr;
    assign enq_sram_rd_addr = enq_bypass ? npr_r_data[2 +: ADDR_W] : free_head_next2_q;

    logic [ADDR_W-1:0] deq_sram_rd_addr;
    assign deq_sram_rd_addr = deq_pend_same_q ? npr_r_data[2 +: ADDR_W]
                                              : q_head_next2_q[lle_deq_queue_id];

    always_comb begin
        npr_r_en   = 1'b0;
        npr_r_addr = '0;
        npr_w_en   = 1'b0;
        npr_w_addr = '0;
        npr_w_data = '0;

        if (build_active) begin
            npr_w_en   = 1'b1;
            npr_w_addr = build_addr;
            npr_w_data = build_wdata;
        end
        else if (deq_grant && deq_need_sram) begin
            npr_r_en   = 1'b1;
            npr_r_addr = deq_sram_rd_addr;
        end
        else if (enq_grant) begin
            npr_r_en   = 1'b1;
            npr_r_addr = enq_sram_rd_addr;
            // 单播按 queue tail 挂链；多播按当前帧槽 tail 挂成独立小链。
            if (enq_is_uni && (q_cell_cnt_q[lle_alloc_queue_id] != '0)) begin
                npr_w_en   = 1'b1;
                npr_w_addr = q_tail_q[lle_alloc_queue_id];
                npr_w_data = {enq_cell,
                              q_tail_ph_q[lle_alloc_queue_id],
                              q_tail_pt_q[lle_alloc_queue_id]};
            end
            else if (!enq_is_uni && !lle_set_pkt_head) begin
                npr_w_en   = 1'b1;
                npr_w_addr = mc_tail_q[mc_enq_slot_c];
                npr_w_data = {enq_cell, 1'b0, 1'b0};
            end
        end
        else if (rcy_grant) begin
            npr_w_en   = 1'b1;
            npr_w_addr = free_tail_q;
            npr_w_data = {rcy_cell, 1'b0, 1'b0};
        end
        else if (agf_splice_gnt && (free_cnt_q != '0)) begin
            // 一次写把待冲刷整链追加到当前 free 链尾。
            npr_w_en   = 1'b1;
            npr_w_addr = free_tail_q;
            npr_w_data = {agf_head_q, 1'b0, 1'b0};
        end
    end

    // 为保证 free/occupancy 批量计数原子更新，拼接拍不与普通 alloc/free 同拍。
    assign agf_splice_gnt = (agf_st_q == AGF_SPLICE) & ~build_active &
                            ~enq_grant & ~rcy_grant & ~do_push;

    next_ptr_sram_1r1w #(.CELL_NUM(CELL_NUM), .DATA_W(ENTRY_W)) u_npr (
        .clk_core(clk_core), .rst_core_n(rst_core_n),
        .r_en(npr_r_en), .r_addr(npr_r_addr), .r_data(npr_r_data),
        .w_en(npr_w_en), .w_addr(npr_w_addr), .w_data(npr_w_data)
    );

    //========================================================================
    // Pend 流水寄存器
    //========================================================================
    always_ff @(posedge clk_core or negedge rst_core_n) begin
        if (!rst_core_n) begin
            deq_pend_q     <= 1'b0;
            deq_pend_qid_q <= '0;
            enq_pend_q     <= 1'b0;
            deq_pend_tail_q    <= 1'b0;
            deq_pend_tail_ph_q <= 1'b0;
            deq_pend_tail_pt_q <= 1'b0;
        end
        else begin
            deq_pend_q     <= deq_grant & deq_need_sram;
            deq_pend_qid_q <= lle_deq_queue_id;
            enq_pend_q     <= enq_grant;
            deq_pend_tail_q    <= (deq_grant & deq_need_sram) &
                                  (deq_sram_rd_addr == q_tail_q[lle_deq_queue_id]);
            deq_pend_tail_ph_q <= q_tail_ph_q[lle_deq_queue_id];
            deq_pend_tail_pt_q <= q_tail_pt_q[lle_deq_queue_id];
        end
    end

    //========================================================================
    // ★ 统一还链: 多播 cell 命中判定 (组合)
    //   QM 逐 cell 还链, 用 lle_free_addr 与本帧 mc_cells_q[0..mc_ncell_q-1] 并行比较。
    //   命中某 slot 且该 slot ref>0 → 视为多播还链, 该 slot cnt--; 减到 0 才真正 push。
    //   未命中 → 按单播还链 (直接 push, occ 用 lle_free_queue_id)。
    //   注: lle_free_is_mcast 仅作提示, 实际以地址匹配为准 (更鲁棒)。
    //========================================================================
    always_comb begin
        mc_rcy_hit     = 1'b0;
        mc_rcy_hit_slot = '0;
        mc_rcy_hit_idx = '0;
        for (int slot_idx = 0; slot_idx < MAX_MC_FRAMES; slot_idx++) begin : MC_RCY_SLOT_MATCH
            for (int cell_idx = 0; cell_idx < MAX_MC_CELLS; cell_idx++) begin : MC_RCY_CELL_MATCH
                if (mc_valid_q[slot_idx] && (MC_IDX_W'(cell_idx) < mc_ncell_q[slot_idx]) &&
                    (mc_cells_q[slot_idx][cell_idx] == lle_free_addr) &&
                    (mc_ref_cnt_q[slot_idx][cell_idx] != '0) && !mc_rcy_hit) begin
                    mc_rcy_hit      = 1'b1;
                    mc_rcy_hit_slot = MC_SLOT_W'(slot_idx);
                    mc_rcy_hit_idx  = MC_IDX_W'(cell_idx);
                end
            end
        end
    end
    // 命中 slot 递减后是否归 0 (=1 → 本 cell 是该 slot 最后一次还链, 真正 push 回 free)
    assign mc_rcy_last = mc_rcy_hit &&
                         (mc_ref_cnt_q[mc_rcy_hit_slot][mc_rcy_hit_idx] == MC_REF_W'(1));

    // 多播还链下溢: 请求声称多播 (is_mcast) 但地址在本帧未命中任何 ref>0 的 slot
    assign mcast_underflow = lle_free_grant & lle_free_is_mcast & (|mc_valid_q) & ~mc_rcy_hit;

    // 整帧全部 cell 已还 (所有 ref_cnt==0) → 清多播槽。
    // mc_ncell_q 在 EOF 拍锁定；EOF 前禁止把正在 build 的多播帧误判为空帧释放。
    logic [MAX_MC_FRAMES-1:0] mc_all_freed;
    logic mc_chain_intact;
    always_comb begin
        mc_all_freed   = '0;
        mc_chain_intact = (|mc_valid_q);
        for (int slot_idx = 0; slot_idx < MAX_MC_FRAMES; slot_idx++) begin
            mc_all_freed[slot_idx] = mc_valid_q[slot_idx] & (mc_ncell_q[slot_idx] != '0);
            if (mc_valid_q[slot_idx] && (mc_ncell_q[slot_idx] == '0))
                mc_chain_intact = 1'b0;
            for (int cell_idx = 0; cell_idx < MAX_MC_CELLS; cell_idx++) begin
                if (MC_IDX_W'(cell_idx) < mc_ncell_q[slot_idx]) begin
                    if (mc_ref_cnt_q[slot_idx][cell_idx] != '0)
                        mc_all_freed[slot_idx] = 1'b0;
                    else
                        mc_chain_intact = 1'b0;
                end
            end
        end
    end

    //========================================================================
    // ★ 老化冲刷整链拼接 FSM
    //   AGF_IDLE   : 锁存待冲刷队列的 head/tail/count/预取快照。
    //   AGF_SPLICE : 等待写口与普通 alloc/free 空闲，一拍完成整链拼接及批量计数更新。
    //   AGF_DONE   : 拉 age_flush_done 一拍后返回 IDLE。
    //========================================================================
    always_ff @(posedge clk_core or negedge rst_core_n) begin
        if (!rst_core_n) begin
            agf_st_q          <= AGF_IDLE;
            agf_qid_q         <= '0;
            agf_head_q        <= '0;
            agf_tail_q        <= '0;
            agf_head_next_q   <= '0;
            agf_head_next2_q  <= '0;
            agf_cell_num_q    <= '0;
            agf_mc_slot_q     <= '0;
            age_flush_done    <= 1'b0;
        end
        else begin
            age_flush_done <= 1'b0;   // 默认脉冲拉低
            unique case (agf_st_q)
                AGF_IDLE: begin
                    // 仅在没有普通队列状态更新、且不处于连续入队包中间时取快照。
                    if (age_flush_req && ~enq_grant && ~deq_grant && ~enq_pkt_lock_q &&
                        ((age_flush_qid != MC_QID[QID_W-1:0]) || mc_chain_intact) &&
                        (q_cell_cnt_q[age_flush_qid] != '0)) begin
                        agf_qid_q        <= age_flush_qid;
                        agf_head_q       <= (age_flush_qid == MC_QID[QID_W-1:0])
                                            ? mc_head_q[mc_age_slot_c] : q_head_q[age_flush_qid];
                        agf_tail_q       <= (age_flush_qid == MC_QID[QID_W-1:0])
                                            ? mc_tail_q[mc_age_slot_c] : q_tail_q[age_flush_qid];
                        agf_head_next_q  <= (age_flush_qid == MC_QID[QID_W-1:0])
                                            ? ((mc_ncell_q[mc_age_slot_c] > 1)
                                               ? mc_cells_q[mc_age_slot_c][1] : mc_head_q[mc_age_slot_c])
                                            : q_head_next_q[age_flush_qid];
                        agf_head_next2_q <= (age_flush_qid == MC_QID[QID_W-1:0])
                                            ? ((mc_ncell_q[mc_age_slot_c] > 2)
                                               ? mc_cells_q[mc_age_slot_c][2] : mc_tail_q[mc_age_slot_c])
                                            : q_head_next2_q[age_flush_qid];
                        agf_cell_num_q   <= (age_flush_qid == MC_QID[QID_W-1:0])
                                            ? CNT_W'(mc_ncell_q[mc_age_slot_c])
                                            : q_cell_cnt_q[age_flush_qid];
                        agf_mc_slot_q    <= mc_age_slot_c;
                        agf_st_q          <= AGF_SPLICE;
                    end
                    else if (age_flush_req && ~enq_grant && ~deq_grant && ~enq_pkt_lock_q &&
                             ((age_flush_qid != MC_QID[QID_W-1:0]) || mc_chain_intact ||
                              (q_cell_cnt_q[age_flush_qid] == '0))) begin
                        // 队列已空: 直接完成
                        age_flush_done <= 1'b1;
                    end
                end
                AGF_SPLICE: begin
                    if (agf_splice_gnt)
                        agf_st_q <= AGF_DONE;
                end
                AGF_DONE: begin
                    // MC_QID 可包含多个独立帧链：在同一次 flush 中逐槽整链拼接，
                    // 直到聚合队列真正为空才对 aging_ctrl 报 done。
                    if ((agf_qid_q == MC_QID[QID_W-1:0]) &&
                        (q_cell_cnt_q[MC_QID] != '0) && mc_age_slot_vld) begin
                        agf_head_q       <= mc_head_q[mc_age_slot_c];
                        agf_tail_q       <= mc_tail_q[mc_age_slot_c];
                        agf_head_next_q  <= (mc_ncell_q[mc_age_slot_c] > 1)
                                            ? mc_cells_q[mc_age_slot_c][1] : mc_head_q[mc_age_slot_c];
                        agf_head_next2_q <= (mc_ncell_q[mc_age_slot_c] > 2)
                                            ? mc_cells_q[mc_age_slot_c][2] : mc_tail_q[mc_age_slot_c];
                        agf_cell_num_q   <= CNT_W'(mc_ncell_q[mc_age_slot_c]);
                        agf_mc_slot_q    <= mc_age_slot_c;
                        agf_st_q         <= AGF_SPLICE;
                    end
                    else begin
                        age_flush_done <= 1'b1;
                        agf_st_q       <= AGF_IDLE;
                    end
                end
                default: begin
                    agf_st_q          <= AGF_IDLE;
                    agf_qid_q         <= '0;
                    agf_head_q        <= '0;
                    agf_tail_q        <= '0;
                    agf_head_next_q   <= '0;
                    agf_head_next2_q  <= '0;
                    agf_cell_num_q    <= '0;
                    agf_mc_slot_q     <= '0;
                    age_flush_done    <= 1'b0;
                end
            endcase
        end
    end

    //========================================================================
    // 主状态更新
    //========================================================================
    logic uni_pkt_tail_deq;    // 本拍出队的是一个真实单播包尾
    assign uni_pkt_tail_deq = deq_grant & ~mc_take_deq & q_head_pt_q[lle_deq_queue_id];

    logic [QUEUE_NUM-1:0] pkt_backlog_inc;
    logic [QUEUE_NUM-1:0] pkt_backlog_dec;
    always_comb begin
        pkt_backlog_inc = '0;
        pkt_backlog_dec = '0;

        if (enq_grant && lle_set_pkt_tail) begin
            if (enq_is_uni) begin
                pkt_backlog_inc[lle_alloc_queue_id] = 1'b1;
            end
            else begin
                for (int port_idx = 0; port_idx < PORT_NUM; port_idx++) begin : MC_PKT_INC_PORT
                    if (lle_set_pkt_head ? lle_alloc_mcast_bitmap[port_idx]
                                         : mc_dst_bitmap_q[mc_enq_slot_c][port_idx])
                        pkt_backlog_inc[lle_set_pkt_head ? carry_qid_c[port_idx]
                                                         : mc_carry_qid_q[mc_enq_slot_c][port_idx]] = 1'b1;
                end
            end
        end

        if (uni_pkt_tail_deq) begin
            pkt_backlog_dec[lle_deq_queue_id] = 1'b1;
        end
        else if (deq_grant && mc_take_deq &&
                 ((mc_rd_idx_q[mc_deq_slot_c][deq_port] + 1'b1) == mc_ncell_q[mc_deq_slot_c])) begin
            pkt_backlog_dec[mc_carry_qid_q[mc_deq_slot_c][deq_port]] = 1'b1;
        end
    end

    always_ff @(posedge clk_core or negedge rst_core_n) begin
        if (!rst_core_n) begin
            for (int q_idx = 0; q_idx < QUEUE_NUM; q_idx++) begin : Q_STATE_RST
                q_head_q[q_idx]            <= '0;
                q_tail_q[q_idx]            <= '0;
                q_cell_cnt_q[q_idx]        <= '0;
                q_head_ph_q[q_idx]         <= 1'b0;
                q_head_pt_q[q_idx]         <= 1'b0;
                q_head_next_q[q_idx]       <= '0;
                q_head_next_ph_q[q_idx]    <= 1'b0;
                q_head_next_pt_q[q_idx]    <= 1'b0;
                q_head_next2_q[q_idx]      <= '0;
                q_tail_ph_q[q_idx]         <= 1'b0;
                q_tail_pt_q[q_idx]         <= 1'b0;
                q_uni_pkt_backlog_q[q_idx] <= '0;
            end
            free_head_q       <= '0;
            free_tail_q       <= '0;
            free_cnt_q        <= '0;
            free_head_next_q  <= '0;
            free_head_next2_q <= '0;
            rcy_fifo_cnt_q    <= '0;
            rcy_fifo_wptr_q   <= '0;
            rcy_fifo_rptr_q   <= '0;
            for (int fifo_idx = 0; fifo_idx < RCY_FIFO_DEPTH; fifo_idx++) begin : RCY_FIFO_RST
                rcy_fifo_mem[fifo_idx] <= '0;
            end
            // 多播槽复位
            mc_valid_q      <= '0;
            mc_build_slot_q <= '0;
            for (int slot_idx = 0; slot_idx < MAX_MC_FRAMES; slot_idx++) begin : MC_SLOT_RST
                mc_dst_bitmap_q[slot_idx] <= '0;
                mc_ncell_q[slot_idx]      <= '0;
                mc_wr_idx_q[slot_idx]     <= '0;
                mc_head_q[slot_idx]       <= '0;
                mc_tail_q[slot_idx]       <= '0;
                mc_dst_cnt_q[slot_idx]    <= '0;
                for (int port_idx = 0; port_idx < PORT_NUM; port_idx++) begin : MC_PORT_RST
                    mc_carry_qid_q[slot_idx][port_idx] <= '0;
                    mc_rd_idx_q[slot_idx][port_idx]    <= '0;
                    mc_rd_done_q[slot_idx][port_idx]   <= 1'b0;
                    mc_pend_uni_q[slot_idx][port_idx]  <= '0;
                end
                for (int cell_idx = 0; cell_idx < MAX_MC_CELLS; cell_idx++) begin : MC_CELLS_RST
                    mc_cells_q[slot_idx][cell_idx]   <= '0;
                    mc_ref_cnt_q[slot_idx][cell_idx] <= '0;
                end
            end
        end
        else if (build_st_q == ST_DONE) begin
            for (int q_idx = 0; q_idx < QUEUE_NUM; q_idx++) begin : Q_STATE_INIT
                q_head_q[q_idx]            <= '0;
                q_tail_q[q_idx]            <= '0;
                q_cell_cnt_q[q_idx]        <= '0;
                q_head_ph_q[q_idx]         <= 1'b0;
                q_head_pt_q[q_idx]         <= 1'b0;
                q_head_next_q[q_idx]       <= '0;
                q_head_next_ph_q[q_idx]    <= 1'b0;
                q_head_next_pt_q[q_idx]    <= 1'b0;
                q_head_next2_q[q_idx]      <= '0;
                q_tail_ph_q[q_idx]         <= 1'b0;
                q_tail_pt_q[q_idx]         <= 1'b0;
                q_uni_pkt_backlog_q[q_idx] <= '0;
            end
            free_head_q       <= '0;
            free_head_next_q  <= {{(ADDR_W-1){1'b0}}, 1'b1};
            free_head_next2_q <= {{(ADDR_W-2){1'b0}}, 2'b10};
            free_tail_q       <= CELL_NUM[ADDR_W-1:0] - 1'b1;
            free_cnt_q        <= CELL_NUM[CNT_W-1:0];
            rcy_fifo_cnt_q  <= '0;
            rcy_fifo_wptr_q <= '0;
            rcy_fifo_rptr_q <= '0;
            mc_valid_q      <= '0;
            mc_build_slot_q <= '0;
            for (int slot_idx = 0; slot_idx < MAX_MC_FRAMES; slot_idx++) begin : MC_SLOT_INIT
                mc_dst_bitmap_q[slot_idx] <= '0;
                mc_ncell_q[slot_idx]      <= '0;
                mc_wr_idx_q[slot_idx]     <= '0;
                mc_head_q[slot_idx]       <= '0;
                mc_tail_q[slot_idx]       <= '0;
                mc_dst_cnt_q[slot_idx]    <= '0;
                for (int port_idx = 0; port_idx < PORT_NUM; port_idx++) begin : MC_PORT_INIT
                    mc_carry_qid_q[slot_idx][port_idx] <= '0;
                    mc_rd_idx_q[slot_idx][port_idx]    <= '0;
                    mc_rd_done_q[slot_idx][port_idx]   <= 1'b0;
                    mc_pend_uni_q[slot_idx][port_idx]  <= '0;
                end
                for (int cell_idx = 0; cell_idx < MAX_MC_CELLS; cell_idx++) begin : MC_REF_INIT
                    mc_cells_q[slot_idx][cell_idx]   <= '0;
                    mc_ref_cnt_q[slot_idx][cell_idx] <= '0;
                end
            end
        end
        else begin
            //================================================================
            // ENQ 落地
            //================================================================
            if (enq_grant) begin
                // free 链两级预取推进 (单播/多播都消耗 free)
                free_head_q <= free_head_next_q;
                if (enq_bypass) free_head_next_q <= npr_r_data[2 +: ADDR_W];
                else            free_head_next_q <= free_head_next2_q;

                //-------- 单播队列挂链 --------
                if (enq_is_uni) begin
                    q_tail_q[lle_alloc_queue_id] <= enq_cell;
                    if (q_cell_cnt_q[lle_alloc_queue_id] == '0) begin
                        q_head_q[lle_alloc_queue_id]         <= enq_cell;
                        q_head_ph_q[lle_alloc_queue_id]      <= lle_set_pkt_head;
                        q_head_pt_q[lle_alloc_queue_id]      <= lle_set_pkt_tail;
                        q_head_next_q[lle_alloc_queue_id]    <= free_head_next_q;
                        q_head_next_ph_q[lle_alloc_queue_id] <= 1'b0;
                        q_head_next_pt_q[lle_alloc_queue_id] <= 1'b0;
                    end
                    else if (q_cell_cnt_q[lle_alloc_queue_id] == 1) begin
                        q_head_next_q[lle_alloc_queue_id]    <= enq_cell;
                        q_head_next_ph_q[lle_alloc_queue_id] <= lle_set_pkt_head;
                        q_head_next_pt_q[lle_alloc_queue_id] <= lle_set_pkt_tail;
                        q_head_next2_q[lle_alloc_queue_id]   <= free_head_next_q;
                    end
                    else if (q_cell_cnt_q[lle_alloc_queue_id] == 2) begin
                        q_head_next2_q[lle_alloc_queue_id]   <= enq_cell;
                    end
                    q_tail_ph_q[lle_alloc_queue_id] <= lle_set_pkt_head;
                    q_tail_pt_q[lle_alloc_queue_id] <= lle_set_pkt_tail;
                end

                if (!enq_is_uni) begin
                    //-------- 多播: 额外写 cell-list 镜像 (读加速) + 建槽 --------
                    mc_cells_q[mc_enq_slot_c][mc_wr_idx_q[mc_enq_slot_c]] <= enq_cell;
                    mc_tail_q[mc_enq_slot_c] <= enq_cell;
                    if (lle_set_pkt_head) begin
                        // SOF: 建槽 + 逐端口快照插入位置
                        mc_valid_q[mc_enq_slot_c]      <= 1'b1;
                        mc_dst_bitmap_q[mc_enq_slot_c] <= lle_alloc_mcast_bitmap;
                        mc_dst_cnt_q[mc_enq_slot_c]    <= mc_dst_cnt_c;
                        mc_wr_idx_q[mc_enq_slot_c]     <= MC_IDX_W'(1);
                        mc_build_slot_q                <= mc_enq_slot_c;
                        mc_head_q[mc_enq_slot_c]       <= enq_cell;
                        for (int port_idx = 0; port_idx < PORT_NUM; port_idx++) begin : MC_PORT_SOF_SNAPSHOT
                            mc_rd_idx_q[mc_enq_slot_c][port_idx]  <= '0;
                            mc_rd_done_q[mc_enq_slot_c][port_idx] <= ~lle_alloc_mcast_bitmap[port_idx];
                            // 承载单播队列号 = 端口*TC_NUM + 该端口多播承载 TC
                            mc_carry_qid_q[mc_enq_slot_c][port_idx] <= carry_qid_c[port_idx];
                            // 快照当前队列的前序完整包；同拍出队的包已不应计入。
                            mc_pend_uni_q[mc_enq_slot_c][port_idx] <=
                                q_uni_pkt_backlog_q[carry_qid_c[port_idx]] -
                                CNT_W'(pkt_backlog_dec[carry_qid_c[port_idx]]);
                        end
                    end
                    else begin
                        mc_wr_idx_q[mc_enq_slot_c] <= mc_wr_idx_q[mc_enq_slot_c] + 1'b1;
                    end
                    if (lle_set_pkt_tail) begin
                        // EOF: 锁定 cell 数
                        mc_ncell_q[mc_enq_slot_c] <= lle_set_pkt_head ? MC_IDX_W'(1)
                                                                      : (mc_wr_idx_q[mc_enq_slot_c] + 1'b1);
                    end
                    // ★ 本 cell 的 ref_cnt 初值 = 目的端口数 N (popcount bitmap)。
                    //   SOF 与后续 cell 都写: SOF 时 bitmap 有效, mc_dst_cnt_c 已算好;
                    //   后续 cell 用同一帧 (bitmap 不变) 的 mc_dst_cnt_c。
                    mc_ref_cnt_q[mc_enq_slot_c][mc_wr_idx_q[mc_enq_slot_c]] <=
                        lle_set_pkt_head ? mc_dst_cnt_c : mc_dst_cnt_q[mc_enq_slot_c];
                end
            end

            // enq_pend T+1: 回填 free_head_next2
            if (enq_pend_q) free_head_next2_q <= npr_r_data[2 +: ADDR_W];

            //================================================================
            // DEQ 落地
            //================================================================
            if (deq_grant) begin
                if (mc_take_deq) begin
                    //-------- 多播 take: 推进该端口读索引 --------
                    if ((mc_rd_idx_q[mc_deq_slot_c][deq_port] + 1'b1) == mc_ncell_q[mc_deq_slot_c])
                        mc_rd_done_q[mc_deq_slot_c][deq_port] <= 1'b1;
                    mc_rd_idx_q[mc_deq_slot_c][deq_port] <=
                        mc_rd_idx_q[mc_deq_slot_c][deq_port] + 1'b1;
                end
                else begin
                    //-------- 单播走链 (两级预取) --------
                    q_head_q[lle_deq_queue_id] <= q_head_next_q[lle_deq_queue_id];
                    if (deq_pend_same_q) begin
                        if (deq_pend_tail_q) begin
                            q_head_ph_q[lle_deq_queue_id] <= deq_pend_tail_ph_q;
                            q_head_pt_q[lle_deq_queue_id] <= deq_pend_tail_pt_q;
                        end
                        else begin
                            q_head_ph_q[lle_deq_queue_id] <= npr_r_data[PH_BIT];
                            q_head_pt_q[lle_deq_queue_id] <= npr_r_data[PT_BIT];
                        end
                        q_head_next_q[lle_deq_queue_id] <= npr_r_data[2 +: ADDR_W];
                    end
                    else begin
                        q_head_ph_q[lle_deq_queue_id] <= q_head_next_ph_q[lle_deq_queue_id];
                        q_head_pt_q[lle_deq_queue_id] <= q_head_next_pt_q[lle_deq_queue_id];
                        q_head_next_q[lle_deq_queue_id] <= q_head_next2_q[lle_deq_queue_id];
                    end

                end
            end

            // 任一承载队列出完一包，所有位于其后的多播槽都向前推进一个包位。
            for (int slot_idx = 0; slot_idx < MAX_MC_FRAMES; slot_idx++) begin : MC_PEND_UPDATE
                for (int port_idx = 0; port_idx < PORT_NUM; port_idx++) begin : MC_PEND_PORT_UPDATE
                    if (mc_valid_q[slot_idx] && mc_dst_bitmap_q[slot_idx][port_idx] &&
                        (mc_pend_uni_q[slot_idx][port_idx] != '0) &&
                        pkt_backlog_dec[mc_carry_qid_q[slot_idx][port_idx]])
                        mc_pend_uni_q[slot_idx][port_idx] <=
                            mc_pend_uni_q[slot_idx][port_idx] - 1'b1;
                end
            end

            // 完整包 backlog 统一维护: EOF +1, pkt_tail -1; 同队同拍加减净不变。
            for (int q_idx = 0; q_idx < PORT_NUM*TC_NUM; q_idx++) begin : PKT_BACKLOG_UPDATE
                unique case ({pkt_backlog_inc[q_idx], pkt_backlog_dec[q_idx]})
                    2'b10: q_uni_pkt_backlog_q[q_idx] <= q_uni_pkt_backlog_q[q_idx] + 1'b1;
                    2'b01: if (q_uni_pkt_backlog_q[q_idx] != '0)
                               q_uni_pkt_backlog_q[q_idx] <= q_uni_pkt_backlog_q[q_idx] - 1'b1;
                    default: ;
                endcase
            end

            // deq_pend T+1: 回填 next_ph/pt 和 next2
            if (deq_pend_q) begin
                if (deq_pend_tail_q) begin
                    q_head_next_ph_q[deq_pend_qid_q] <= deq_pend_tail_ph_q;
                    q_head_next_pt_q[deq_pend_qid_q] <= deq_pend_tail_pt_q;
                end
                else begin
                    q_head_next_ph_q[deq_pend_qid_q] <= npr_r_data[PH_BIT];
                    q_head_next_pt_q[deq_pend_qid_q] <= npr_r_data[PT_BIT];
                    q_head_next2_q[deq_pend_qid_q]   <= npr_r_data[2 +: ADDR_W];
                end
            end

            //================================================================
            // q_cell_cnt: 入队 (单播链或多播链 MC_QID) +1; 出队 (仅真实单播走链) -1。
            //   多播出队 (mc_take) 不推进 MC_QID 队头、不减 MC_QID cnt (多端口共享读);
            //   MC_QID cnt 只在整帧还链完成时清 0 (见下面 release 分支)。
            //   注: 多播 alloc 到 MC_QID, 与单播 deq 到某单播 qid, 二者 qid 必不同 →
            //       无 "同 queue 同拍" 冲突; 仅单播自环 (alloc==deq) 需净不变处理。
            //================================================================
            if (enq_grant && enq_is_uni && deq_grant && ~mc_take_deq &&
                (lle_alloc_queue_id == lle_deq_queue_id)) begin
                // 同 (单播) queue 同拍入+出: 净不变
            end
            else begin
                if (enq_grant && enq_is_uni)
                    q_cell_cnt_q[lle_alloc_queue_id] <= q_cell_cnt_q[lle_alloc_queue_id] + 1'b1;
                if (deq_grant && ~mc_take_deq)
                    q_cell_cnt_q[lle_deq_queue_id]   <= q_cell_cnt_q[lle_deq_queue_id]   - 1'b1;
            end

            // MC_QID 只作多槽聚合占用计数：分配 +1，某 cell 最后一个引用回收时 -1。
            unique case ({enq_grant && !enq_is_uni,
                          mc_rcy_hit && mc_rcy_last && lle_free_req &&
                          !rcy_fifo_full && !build_active && !agf_active})
                2'b10: q_cell_cnt_q[MC_QID] <= q_cell_cnt_q[MC_QID] + 1'b1;
                2'b01: q_cell_cnt_q[MC_QID] <= q_cell_cnt_q[MC_QID] - 1'b1;
                default: ;
            endcase

            //================================================================
            // ★ 统一还链: 多播 cell ref-count 递减
            //   本拍还链请求命中多播槽某 slot (mc_rcy_hit) → 该 slot ref_cnt--。
            //   (是否 push 由 ext_free_push 决定: mc_rcy_last 时才真正 push 到 free 链。)
            //   递减在 lle_free_req 有效且命中即执行, 与 push 是否被 FIFO 满阻塞无关?
            //   —— 为与 push/occ 一致, 仅当本拍确实受理 (ext_free_push 或未满) 时递减。
            //   这里: 命中且请求被受理 (~rcy_fifo_full & ~build_active) → 递减。
            //================================================================
            if (mc_rcy_hit && lle_free_req && ~rcy_fifo_full && ~build_active && ~agf_active) begin
                mc_ref_cnt_q[mc_rcy_hit_slot][mc_rcy_hit_idx] <=
                    mc_ref_cnt_q[mc_rcy_hit_slot][mc_rcy_hit_idx] - 1'b1;
            end

            //================================================================
            // ★ 多播整帧释放: 当本帧所有 cell 的 ref_cnt 都已归 0 (mc_all_freed)
            //   → 清多播槽, 允许收下一条多播帧。
            //   注: mc_all_freed 用当拍寄存器值判断; 最后一个 cell 的递减在本拍生效,
            //       下一拍 mc_all_freed 才为真 → 下一拍清槽 (无需再走 walk FSM)。
            //================================================================
            for (int slot_idx = 0; slot_idx < MAX_MC_FRAMES; slot_idx++) begin : MC_SLOT_RELEASE
                if (mc_all_freed[slot_idx]) begin
                    mc_valid_q[slot_idx]      <= 1'b0;
                    mc_dst_bitmap_q[slot_idx] <= '0;
                    mc_ncell_q[slot_idx]      <= '0;
                    mc_wr_idx_q[slot_idx]     <= '0;
                    mc_head_q[slot_idx]       <= '0;
                    mc_tail_q[slot_idx]       <= '0;
                    mc_dst_cnt_q[slot_idx]    <= '0;
                    for (int port_idx = 0; port_idx < PORT_NUM; port_idx++) begin : MC_PORT_RELEASE_CLR
                        mc_rd_done_q[slot_idx][port_idx] <= 1'b0;
                        mc_pend_uni_q[slot_idx][port_idx] <= '0;
                        mc_rd_idx_q[slot_idx][port_idx] <= '0;
                    end
                    for (int cell_idx = 0; cell_idx < MAX_MC_CELLS; cell_idx++) begin : MC_REF_RELEASE_CLR
                        mc_ref_cnt_q[slot_idx][cell_idx] <= '0;
                    end
                end
            end

            //================================================================
            // Recycle FIFO push + pop
            //================================================================
            if (do_push) begin
                rcy_fifo_mem[rcy_fifo_wptr_q] <= push_cell;
                rcy_fifo_wptr_q <= rcy_fifo_wptr_q + 1'b1;
            end
            if (do_pop) begin
                rcy_fifo_rptr_q <= rcy_fifo_rptr_q + 1'b1;
                free_tail_q     <= rcy_cell;
            end

            unique case ({do_push, do_pop})
                2'b10:   rcy_fifo_cnt_q <= rcy_fifo_cnt_q + 1'b1;
                2'b01:   rcy_fifo_cnt_q <= rcy_fifo_cnt_q - 1'b1;
                default: ;
            endcase

            // free_cnt: 普通路径逐 cell 更新；老化拼接拍一次增加整链 cell 数。
            if (agf_splice_gnt) begin
                free_cnt_q <= free_cnt_q + agf_cell_num_q;
            end
            else begin
                unique case ({enq_grant, do_push})
                    2'b10:   free_cnt_q <= free_cnt_q - 1'b1;
                    2'b01:   free_cnt_q <= free_cnt_q + 1'b1;
                    default: ;
                endcase
            end

            //================================================================
            // ★ 老化整链拼接: 一拍更新 free head/tail/count，并清空原队列寄存器。
            //   agf_active 期间阻塞普通 enq/deq，grant 又与普通 free 错开，因此更新原子化。
            //================================================================
            if (agf_splice_gnt) begin
                if (free_cnt_q == '0) begin
                    free_head_q       <= agf_head_q;
                    free_head_next_q  <= agf_head_next_q;
                    free_head_next2_q <= agf_head_next2_q;
                end
                free_tail_q <= agf_tail_q;

                if (agf_qid_q != MC_QID[QID_W-1:0]) begin
                    q_head_q[agf_qid_q]            <= '0;
                    q_tail_q[agf_qid_q]            <= '0;
                    q_cell_cnt_q[agf_qid_q]        <= '0;
                    q_head_ph_q[agf_qid_q]         <= 1'b0;
                    q_head_pt_q[agf_qid_q]         <= 1'b0;
                    q_head_next_q[agf_qid_q]       <= '0;
                    q_head_next_ph_q[agf_qid_q]    <= 1'b0;
                    q_head_next_pt_q[agf_qid_q]    <= 1'b0;
                    q_head_next2_q[agf_qid_q]      <= '0;
                    q_tail_ph_q[agf_qid_q]         <= 1'b0;
                    q_tail_pt_q[agf_qid_q]         <= 1'b0;
                    q_uni_pkt_backlog_q[agf_qid_q] <= '0;
                end

                // 冲刷多播专用队列: 每次整链释放一个完整帧槽。
                if (agf_qid_q == MC_QID[QID_W-1:0]) begin
                    q_cell_cnt_q[MC_QID] <= q_cell_cnt_q[MC_QID] - agf_cell_num_q;
                    mc_valid_q[agf_mc_slot_q]      <= 1'b0;
                    mc_dst_bitmap_q[agf_mc_slot_q] <= '0;
                    mc_ncell_q[agf_mc_slot_q]      <= '0;
                    mc_wr_idx_q[agf_mc_slot_q]     <= '0;
                    mc_head_q[agf_mc_slot_q]       <= '0;
                    mc_tail_q[agf_mc_slot_q]       <= '0;
                    mc_dst_cnt_q[agf_mc_slot_q]    <= '0;
                    for (int port_idx = 0; port_idx < PORT_NUM; port_idx++) begin : MC_PORT_AGF_CLR
                        if (mc_dst_bitmap_q[agf_mc_slot_q][port_idx] &&
                            !mc_rd_done_q[agf_mc_slot_q][port_idx] &&
                            (q_uni_pkt_backlog_q[mc_carry_qid_q[agf_mc_slot_q][port_idx]] != '0))
                            q_uni_pkt_backlog_q[mc_carry_qid_q[agf_mc_slot_q][port_idx]] <=
                                q_uni_pkt_backlog_q[mc_carry_qid_q[agf_mc_slot_q][port_idx]] - 1'b1;
                        mc_rd_done_q[agf_mc_slot_q][port_idx] <= 1'b0;
                        mc_pend_uni_q[agf_mc_slot_q][port_idx] <= '0;
                        mc_rd_idx_q[agf_mc_slot_q][port_idx] <= '0;
                    end
                    for (int cell_idx = 0; cell_idx < MAX_MC_CELLS; cell_idx++) begin : MC_REF_AGF_CLR
                        mc_ref_cnt_q[agf_mc_slot_q][cell_idx] <= '0;
                    end
                end
            end
        end
    end

    //========================================================================
    // ★ 老化用输出: 队列非空位图 + 出队 fire 喂狗
    //   位宽 QUEUE_NUM (=PORT_NUM*TC_NUM 单播 + 1 多播)
    //   [0..PORT_NUM*TC_NUM-1] : 单播队列 cell 占用 (q_cell_cnt_q[q]!=0)
    //   [MC_QID = QUEUE_NUM-1] : 多播槽占用 (mc_valid_q 或 chain33 SRAM cnt!=0)
    //   用一个覆盖全部 QUEUE_NUM 位的 for 循环, 确保最高位 MC_QID 也被明确赋值。
    //========================================================================
    always_comb begin
        for (int oc = 0; oc < QUEUE_NUM; oc++) begin : Q_OCCUPIED_GEN
            if (oc == MC_QID)
                q_occupied_vec[oc] = (|mc_valid_q) | (q_cell_cnt_q[oc] != '0);
            else
                q_occupied_vec[oc] = (q_cell_cnt_q[oc] != '0);
        end
    end
    assign deq_fire_evt = deq_grant;   // 单播或多播承载队列出队 fire (喂狗)
    assign deq_fire_qid = lle_deq_queue_id;

    //========================================================================
    // 对外组合输出 (含多播 splice 覆盖)
    //========================================================================
    logic [ADDR_W-1:0] mc_cur_cell;
    logic              mc_cur_ph, mc_cur_pt, mc_next_pt;
    logic [MC_IDX_W-1:0] mc_cur_idx_c;
    assign mc_cur_idx_c = (mc_rd_idx_q[mc_deq_slot_c][deq_port] < MAX_MC_CELLS)
                          ? mc_rd_idx_q[mc_deq_slot_c][deq_port] : '0;
    assign mc_cur_cell = mc_cells_q[mc_deq_slot_c][mc_cur_idx_c];
    assign mc_cur_ph   = (mc_rd_idx_q[mc_deq_slot_c][deq_port] == '0);
    assign mc_cur_pt   = ((mc_rd_idx_q[mc_deq_slot_c][deq_port] + MC_IDX_W'(1)) ==
                          mc_ncell_q[mc_deq_slot_c]);
    assign mc_next_pt  = ((mc_rd_idx_q[mc_deq_slot_c][deq_port] + MC_IDX_W'(2)) ==
                          mc_ncell_q[mc_deq_slot_c]);

    assign lle_qhead          = mc_take_deq ? mc_cur_cell : q_head_q[lle_deq_queue_id];
    assign lle_qhead_pkt_head = mc_take_deq ? mc_cur_ph   : q_head_ph_q[lle_deq_queue_id];
    assign lle_qhead_pkt_tail = mc_take_deq ? mc_cur_pt   : q_head_pt_q[lle_deq_queue_id];
    assign lle_qhead_next_pkt_tail = mc_take_deq ? mc_next_pt : q_head_next_pt_q[lle_deq_queue_id];
    // 空: enq packet 锁住期间暂停外部出队; 多播 take 时非空;
    // 否则看单播 cnt (承载队列 pend_uni>0 时必有单播 cell, cnt>0)。
    assign lle_q_empty        = (agf_active | enq_pkt_lock_q) ? 1'b1 :
                                (mc_take_deq ? 1'b0 : (q_cell_cnt_q[lle_deq_queue_id] == '0));

    //========================================================================
    // ★ B2: 32 条常规队列 empty 向量 (给 QM 调度用)
    //   q_empty[q] = ~( 实际单播 cnt[q]!=0  |  该 q 是某目的端口承载队列且完整多播未被该端口读完 )
    //   多播只计入【各目的端口的承载队列】(每端口 1 条), 不计入全部 32 条。
    //========================================================================
    always_comb begin
        for (int qq = 0; qq < PORT_NUM*TC_NUM; qq++) begin : Q_EMPTY_GEN
            automatic int pq = qq / TC_NUM;
            automatic logic mc_here = 1'b0;
            for (int slot_idx = 0; slot_idx < MAX_MC_FRAMES; slot_idx++) begin
                mc_here |= mc_valid_q[slot_idx] && (mc_ncell_q[slot_idx] != '0) &&
                           mc_dst_bitmap_q[slot_idx][pq] &&
                           !mc_rd_done_q[slot_idx][pq] &&
                           (QID_W'(qq) == mc_carry_qid_q[slot_idx][pq]);
            end
            q_empty_vec[qq] = ~((q_cell_cnt_q[qq] != '0) | mc_here);
        end
    end

    //========================================================================
    // ★ 32 条常规队列 "pkt 数为 0" 向量 (给 QM 调度用; 完整包粒度)
    //   q_pkt_empty[q] = ~(该队列完整包 backlog != 0)。
    //   多播 EOF 后已实际计入各目的端口承载队列, 不再额外用 mc_valid 组合虚拟加 1。
    //========================================================================
    always_comb begin
        for (int qq = 0; qq < PORT_NUM*TC_NUM; qq++) begin : Q_PKT_EMPTY_GEN
            q_pkt_empty_vec[qq] = (q_uni_pkt_backlog_q[qq] == '0);
        end
    end

    assign lle_free_grant = lle_free_req & ~build_active & ~rcy_fifo_full & ~agf_active;
    assign lle_free_done  = rcy_grant;

    logic [QID_W-1:0] evt_egress_port_full;
    logic [QID_W-1:0] evt_free_egress_port_full;
    logic [QID_W-1:0] free_evt_qid_c;

    assign evt_egress_port_full      = lle_alloc_queue_id / TC_NUM;
    assign free_evt_qid_c             = agf_splice_gnt ? agf_qid_q : push_qid;
    assign evt_free_egress_port_full = free_evt_qid_c / TC_NUM;

    // alloc 事件 (单播用 alloc_queue_id; 多播用 MC_QID → occ 只在 MC_QID 计一次/cell)
    assign lle_alloc_evt   = enq_grant;
    assign evt_queue_id    = lle_alloc_is_mcast ? MC_QID[QID_W-1:0] : lle_alloc_queue_id;
    assign evt_egress_port = lle_alloc_is_mcast ? '0
                             : evt_egress_port_full[PORT_W-1:0];

    // free 事件: 普通回收为 1 cell；老化整链拼接携带本次批量释放数量。
    assign lle_free_evt          = do_push | agf_splice_gnt;
    assign evt_free_queue_id     = free_evt_qid_c;
    assign evt_free_egress_port  = (free_evt_qid_c < QID_W'(PORT_NUM*TC_NUM))
                                   ? evt_free_egress_port_full[PORT_W-1:0] : '0;
    assign evt_free_cell_num     = agf_splice_gnt ? agf_cell_num_q : CNT_W'(1);

    // 只要存在建链、连续包、SRAM 返回、回收 FIFO 或老化拼链，顶层就不得关钟。
    assign lle_busy = (build_st_q != ST_IDLE) |
                      enq_pkt_lock_q          |
                      deq_pend_q              |
                      enq_pend_q              |
                      ~rcy_fifo_empty         |
                      agf_active;

`ifdef SIM_BEHAVIOR_SRAM
    initial begin
        assert ((MAX_MC_FRAMES >= 2) && (MAX_MC_FRAMES <= 4))
            else $fatal(1, "[lle] MAX_MC_FRAMES must be in range 2..4");
    end
    always_ff @(posedge clk_core) begin
        if (rst_core_n && lle_free_req && rcy_fifo_full && !build_active)
            $warning("[lle] recycle FIFO full: free request ignored");
    end
    always_ff @(posedge clk_core) begin
        if (rst_core_n && enq_grant && (free_cnt_q == '0))
            $error("[lle] free pool underflow: alloc when free_cnt==0");
    end
    always_ff @(posedge clk_core) begin
        if (rst_core_n && mcast_underflow)
            $error("[lle] mcast recycle underflow: addr %0h not in active mcast frame", lle_free_addr);
    end
    always_ff @(posedge clk_core) begin
        if (rst_core_n && enq_grant && lle_alloc_is_mcast && lle_set_pkt_head && !mc_free_slot_vld)
            $error("[lle] mcast enqueue while all slots busy (should be gated by mc_busy)");
    end
    always_ff @(posedge clk_core) begin
        if (rst_core_n && agf_splice_gnt) begin
            if ((agf_cell_num_q == '0) ||
                ((agf_qid_q != MC_QID[QID_W-1:0]) &&
                 (q_cell_cnt_q[agf_qid_q] != agf_cell_num_q)) ||
                ((agf_qid_q == MC_QID[QID_W-1:0]) &&
                 (q_cell_cnt_q[agf_qid_q] < agf_cell_num_q)))
                $error("[lle] aging splice snapshot mismatch: qid=%0d snap=%0d live=%0d",
                       agf_qid_q, agf_cell_num_q, q_cell_cnt_q[agf_qid_q]);
            if ((free_cnt_q + agf_cell_num_q) > CELL_NUM[CNT_W-1:0])
                $error("[lle] aging splice would overflow free count");
        end
    end
`endif

endmodule


//============================================================================
// 1R1W Next-Ptr SRAM 行为模型 (综合时换 vendor 1R1W SRAM)
//   - 1 读口 + 1 写口, 同拍可并行; 同拍读写同一地址: read-first
//============================================================================
module next_ptr_sram_1r1w #(
    parameter int CELL_NUM = 8192,
    parameter int DATA_W   = 15,
    localparam int ADDR_W  = $clog2(CELL_NUM)
)(
    input  logic              clk_core,
    input  logic              rst_core_n,
    input  logic              r_en,
    input  logic [ADDR_W-1:0] r_addr,
    output logic [DATA_W-1:0] r_data,
    input  logic              w_en,
    input  logic [ADDR_W-1:0] w_addr,
    input  logic [DATA_W-1:0] w_data
);
    logic [DATA_W-1:0] mem [CELL_NUM];
    always_ff @(posedge clk_core or negedge rst_core_n) begin
        if (!rst_core_n) r_data <= '0;
        else if (r_en)   r_data <= mem[r_addr];
    end
    always_ff @(posedge clk_core) begin
        if (w_en) mem[w_addr] <= w_data;
    end
endmodule
