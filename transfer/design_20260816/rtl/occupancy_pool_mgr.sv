...
module occupancy_pool_mgr #(
    ...
    input  logic [CNT_W-1:0]           occ_free_cell_num,    // 回收数量；普通=1，老化整链=链长
    ...
    logic [CNT_W-1:0]     q_static_free_num [QUEUE_NUM];
    ...
    always_comb begin
        ...
        same_queue_evt = lle_alloc_evt && occ_free_vld &&
                         (evt_queue_id == occ_free_queue_id);
        same_port_evt  = lle_alloc_evt && occ_free_vld && (alloc_port == free_port);
        // free: 校验队列当前占用足以覆盖本次释放数量。
        for (int i = 0; i < QUEUE_NUM; i++) begin : FREE_ALLOWED_Q_SCAN
            if (occ_free_vld && (occ_free_cell_num != '0) &&
                (occ_free_queue_id == i) && (q_cell_cnt_q[i] >= occ_free_cell_num))
                free_allowed = 1'b1;
        end
        ...
        q_static_inc = '0;
        q_static_dec = '0;
        for (int i = 0; i < QUEUE_NUM; i++) begin : Q_INC_DEC_GEN
            ...
            q_static_free_num[i] = (q_static_used_q[i] < occ_free_cell_num)
                                   ? q_static_used_q[i] : occ_free_cell_num;
        end

        port_inc = '0;
        port_dec = '0;
        for (int i = 0; i < PORT_NUM; i++) begin : PORT_INC_DEC_GEN
            ...
            port_inc[i] = alloc_allowed && (evt_queue_id < QID_W'(PORT_NUM*TC_NUM)) &&
                          (alloc_port == i) && !(same_port_evt && free_allowed);
            port_dec[i] = free_allowed && (occ_free_queue_id < QID_W'(PORT_NUM*TC_NUM)) &&
                          (free_port == i) &&
                          (per_port_used_q[i] >= occ_free_cell_num) &&
    ...
    always_ff @(posedge clk_core or negedge rst_core_n) begin
        if (!rst_core_n) begin
            for (int q_idx = 0; q_idx < QUEUE_NUM; q_idx++) begin : Q_CELL_CNT_RST
                q_cell_cnt_q[q_idx] <= '0;
            end
        end
        else if (clr_ptr_cnt) begin              // ★ 初始化期同步清
            for (int q_idx = 0; q_idx < QUEUE_NUM; q_idx++) begin : Q_CELL_CNT_CLR
                q_cell_cnt_q[q_idx] <= '0;
            end
        end
        else begin
            for (int q_idx = 0; q_idx < QUEUE_NUM; q_idx++) begin : Q_CELL_CNT_UPDATE
                unique case ({q_cell_inc[q_idx], q_cell_dec[q_idx]})
                    2'b10: q_cell_cnt_q[q_idx] <= q_cell_cnt_q[q_idx] + CNT_W'(1);
                    2'b01: q_cell_cnt_q[q_idx] <= q_cell_cnt_q[q_idx] - occ_free_cell_num;
                    2'b11: q_cell_cnt_q[q_idx] <= q_cell_cnt_q[q_idx] + CNT_W'(1) - occ_free_cell_num;
                    default: ;
                endcase
            end
        end
    ...
    always_ff @(posedge clk_core or negedge rst_core_n) begin
        if (!rst_core_n) begin
            for (int q_idx = 0; q_idx < QUEUE_NUM; q_idx++) begin : Q_STATIC_USED_RST
                q_static_used_q[q_idx] <= '0;
            end
        end
        else if (clr_ptr_cnt) begin              // ★ 初始化期同步清
            for (int q_idx = 0; q_idx < QUEUE_NUM; q_idx++) begin : Q_STATIC_USED_CLR
                q_static_used_q[q_idx] <= '0;
            end
        end
        else begin
            for (int q_idx = 0; q_idx < QUEUE_NUM; q_idx++) begin : Q_STATIC_USED_UPDATE
                unique case ({q_static_inc[q_idx], q_static_dec[q_idx]})
                    2'b10: q_static_used_q[q_idx] <= q_static_used_q[q_idx] + CNT_W'(1);
                    2'b01: q_static_used_q[q_idx] <= q_static_used_q[q_idx] - q_static_free_num[q_idx];
                    2'b11: q_static_used_q[q_idx] <= q_static_used_q[q_idx] + CNT_W'(1) -
                                                            q_static_free_num[q_idx];
                    default: ;
                endcase
            end
        end
    ...
    always_ff @(posedge clk_core or negedge rst_core_n) begin
        if (!rst_core_n) begin
            for (int port_idx = 0; port_idx < PORT_NUM; port_idx++) begin : PER_PORT_USED_RST
                per_port_used_q[port_idx] <= '0;
            end
        end
        else if (clr_ptr_cnt) begin              // ★ 初始化期同步清
            for (int port_idx = 0; port_idx < PORT_NUM; port_idx++) begin : PER_PORT_USED_CLR
                per_port_used_q[port_idx] <= '0;
            end
        end
        else begin
            for (int port_idx = 0; port_idx < PORT_NUM; port_idx++) begin : PER_PORT_USED_UPDATE
                unique case ({port_inc[port_idx], port_dec[port_idx]})
                    2'b10: per_port_used_q[port_idx] <= per_port_used_q[port_idx] + CNT_W'(1);
                    2'b01: per_port_used_q[port_idx] <= per_port_used_q[port_idx] - occ_free_cell_num;
                    2'b11: per_port_used_q[port_idx] <= per_port_used_q[port_idx] + CNT_W'(1) -
                                                           occ_free_cell_num;
                    default: ;
                endcase
            end
    ...
    always_ff @(posedge clk_core or negedge rst_core_n) begin
        if (!rst_core_n) begin
            ...
            unique case ({alloc_allowed, free_allowed})
                2'b10: if (free_count_q != '0) begin   // 仅分配
                    free_count_q  <= free_count_q  - 1'b1;
                    global_used_q <= global_used_q + 1'b1;
                end
                2'b01: if (global_used_q >= occ_free_cell_num) begin  // 仅回收
                    free_count_q  <= free_count_q  + occ_free_cell_num;
                    global_used_q <= global_used_q - occ_free_cell_num;
                end
                2'b11: begin                           // 同拍分配+回收: 按数量计算净变化
                    free_count_q  <= free_count_q  + occ_free_cell_num - CNT_W'(1);
                    global_used_q <= global_used_q + CNT_W'(1) - occ_free_cell_num;
                end
                default: ;
            endcase
        end
    ...
    //========================================================================
    always_comb begin
        for (int i = 0; i < QUEUE_NUM; i++) begin : Q_MAX_STATUS_GEN
    ...
    always_comb begin
        for (int i = 0; i < PORT_NUM; i++) begin : PORT_MAX_STATUS_GEN
    ...
    always_comb begin
        for (int i = 0; i < PORT_NUM; i++) begin : PAUSE_THRESH_GEN
    ...
    always_ff @(posedge clk_core or negedge rst_core_n) begin
        if (!rst_core_n)
            ...
            for (int i = 0; i < PORT_NUM; i++) begin : PAUSE_REQ_UPDATE
                if      (!cfg_pause_en)    pause_req[i] <= 1'b0;
                else if ( pause_set[i])    pause_req[i] <= 1'b1;
                else if ( pause_clr[i])    pause_req[i] <= 1'b0;
                // 中间区保持原值, 迟滞
            end
        end
    end
    ...
    always_comb begin
        for (int port_idx = 0; port_idx < PORT_NUM; port_idx++) begin : PER_TC_USED_PORT
            for (int tc_idx = 0; tc_idx < TC_NUM; tc_idx++) begin : PER_TC_USED_TC
                per_tc_used[port_idx][tc_idx] = q_cell_cnt_q[port_idx*TC_NUM + tc_idx];
            end
        end
    end
    ...
    always_comb begin
        for (int port_idx = 0; port_idx < PORT_NUM; port_idx++) begin : PFC_THRESH_PORT
            for (int tc_idx = 0; tc_idx < TC_NUM; tc_idx++) begin : PFC_THRESH_TC
                pfc_set[port_idx][tc_idx] = per_tc_used[port_idx][tc_idx] >= cfg_pfc_xoff[port_idx][tc_idx];
                pfc_clr[port_idx][tc_idx] = per_tc_used[port_idx][tc_idx] <  cfg_pfc_xon[port_idx][tc_idx];
            end
        end
    end
    ...
    always_ff @(posedge clk_core or negedge rst_core_n) begin
        if (!rst_core_n)
            ...
            for (int port_idx = 0; port_idx < PORT_NUM; port_idx++) begin : PFC_REQ_PORT
                for (int tc_idx = 0; tc_idx < TC_NUM; tc_idx++) begin : PFC_REQ_TC
                    if      (!cfg_pfc_en)                    pfc_req[port_idx][tc_idx] <= 1'b0;
                    else if ( pfc_set[port_idx][tc_idx])     pfc_req[port_idx][tc_idx] <= 1'b1;
                    else if ( pfc_clr[port_idx][tc_idx])     pfc_req[port_idx][tc_idx] <= 1'b0;
                    // 中间区保持原值, 迟滞
                end
            end
        end
    ...
    always_ff @(posedge clk_core or negedge rst_core_n) begin
        if (!rst_core_n) begin
            for (int i = 0; i < QUEUE_NUM; i++) begin : STATS_CNT_RST_Q
                tail_drop_cnt_q[i]    <= '0;
                q_max_assert_cnt_q[i] <= '0;
            end
            for (int port_idx = 0; port_idx < PORT_NUM; port_idx++) begin : STATS_CNT_RST_PORT
                pause_tx_cnt_q[port_idx] <= '0;
            end
            q_max_reached_d <= '0;
            pause_req_d     <= '0;
        end
        else if (clr_ptr_cnt) begin              // ★ 初始化期同步清
            for (int i = 0; i < QUEUE_NUM; i++) begin : STATS_CNT_CLR_Q
                tail_drop_cnt_q[i]    <= '0;
                q_max_assert_cnt_q[i] <= '0;
            end
            for (int port_idx = 0; port_idx < PORT_NUM; port_idx++) begin : STATS_CNT_CLR_PORT
                pause_tx_cnt_q[port_idx] <= '0;
            end
            q_max_reached_d <= '0;
            pause_req_d     <= '0;
            ...
            // 记录上一拍状态 (做上升沿检测)
            q_max_reached_d <= q_max_reached;
            pause_req_d     <= pause_req;

            // tail_drop: 命中 occ_query_queue_id 队列 +1 (饱和)
            for (int i = 0; i < QUEUE_NUM; i++) begin : TAIL_DROP_CNT_UPDATE
                if (tail_drop_evt && (occ_query_queue_id == QID_W'(i)) &&
                    (tail_drop_cnt_q[i] != '1))
                    tail_drop_cnt_q[i] <= tail_drop_cnt_q[i] + 1'b1;
            end

            // q_max_assert: q_max_reached 由 0→1 +1 (饱和)
            for (int i = 0; i < QUEUE_NUM; i++) begin : Q_MAX_ASSERT_CNT_UPDATE
                if (q_max_reached[i] && !q_max_reached_d[i] &&
                    (q_max_assert_cnt_q[i] != '1))
                    q_max_assert_cnt_q[i] <= q_max_assert_cnt_q[i] + 1'b1;
            end

            // pause_tx: pause_req 由 0→1 +1 (饱和)
            for (int i = 0; i < PORT_NUM; i++) begin : PAUSE_TX_CNT_UPDATE
                if (pause_req[i] && !pause_req_d[i] &&
                    (pause_tx_cnt_q[i] != '1))
                    pause_tx_cnt_q[i] <= pause_tx_cnt_q[i] + 1'b1;
            end
        end
    end
    ...
    always_comb begin
        for (int i = 0; i < QUEUE_NUM; i++) begin : STATS_OUT_Q
            st_q_static_used[i]  = q_static_used_q[i];
            st_per_queue_used[i] = q_cell_cnt_q[i];
        end
        for (int port_idx = 0; port_idx < PORT_NUM; port_idx++) begin : STATS_OUT_PORT
            st_per_port_used[port_idx] = per_port_used_q[port_idx];
        end
    end
    ...
    always_ff @(posedge clk_core or negedge rst_core_n) begin
        if (!rst_core_n) begin
            ...
            underflow_alarm <= ~conserve_ok
                               ...
                               | (occ_free_vld & ~free_allowed)
                               | (free_allowed & (global_used_q < occ_free_cell_num));
        end
    end
