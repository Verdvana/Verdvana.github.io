...
module smmu #(
    ...
    parameter int MAX_MC_FRAMES = 4, // 并发多播帧槽数 (2~4, 寄存器实现)
    parameter int CLK_GATING_EN = 1, // 1: 纯 RTL 门控时钟; 0: 直通 clk_core (对比/调试)
    ...
    // ID 均从 0 连续编码到 NUM-1；NUM=1 时保留 1 bit，避免零位宽向量。
    ...
    input  logic [TC_W-1:0]       enq_queue_id,        // ★ 目标 TC (0..TC_NUM-1); 完整队列=port*TC_NUM+TC
    ...
    input  logic [TC_W-1:0]       deq_queue_id,        // 出队 TC/queue id; 完整队列=port*TC_NUM+TC
    input  logic [PORT_W-1:0]     deq_egress_port,     // 出队端口 ID
    ...
    output logic                  deq_pkt_tail_next,   // 下一拍继续出队时将返回 pkt_tail

    //------------------------------------------------------------------------
    // G4 - 地址回收 / 组播回收接口 (QM → MMU)
    //   ★ recycle_queue_id + recycle_egress_port: QM 提供被回收 cell 所属端口/TC
    //     (QM 有 descriptor)。recycle_ctrl 内部拼成完整 queue_id 后透传给 LLE →
    //     occupancy, 用于 per-queue/port 占用计数 --, 与 LLE free 事件同拍。
    //------------------------------------------------------------------------
    input  logic                  recycle_req,         // 还链请求 (单/多播统一, QM 逐 cell 还)
    input  logic [ADDR_W-1:0]     recycle_cell_addr,   // 待回收 cell 地址
    input  logic [TC_W-1:0]       recycle_queue_id,    // 单播回收所属 TC/queue id (多播命中时忽略)
    input  logic [PORT_W-1:0]     recycle_egress_port, // 单播回收所属端口 ID (多播命中时忽略)
    ...
    logic                  occ_accept, occ_drop, occ_no_free;
    ...
    logic [TC_W-1:0]       lle_alloc_mcast_tc;        // ★ B2 组播帧 TC → LLE
    logic                  mc_busy;                  // ★ B2: 多播帧槽全满

    // Dequeue Ctrl ↔ LLE
    logic [QID_W-1:0]      lle_deq_queue_id;
    logic [ADDR_W-1:0]     lle_qhead;
    logic                  lle_qhead_pkt_head, lle_qhead_pkt_tail, lle_qhead_next_pkt_tail;
    logic                  lle_q_empty;
    logic                  lle_deq_fire;
    ...
    //   free  事件: lle_free_evt + queue/port/cell_num（普通回收=1，老化为整链长度）
    ...
    logic [CNT_W-1:0]      evt_free_cell_num;
    logic                  lle_busy;
    ...
    //========================================================================
    // 纯 RTL 低功耗时钟门控
    //   - 原始外部请求和配置变化直接走常开唤醒路径。
    //   - 队列可以非空；数据通路 pending 排空后连续确认两拍即关钟。
    //   - aging_ctrl 使用常开时钟，超时后用 age_flush_req 唤醒数据通路。
    //   - 有连续流量时 clock_req 始终为 1，smmu_core_clk 与 clk_core 同拍。
    //========================================================================
    logic smmu_core_clk;
    logic cfg_change_req;
    logic wake_req;
    logic core_work_pending;
    logic core_idle_q;
    logic idle_qual_q;
    logic clock_req;

    // 当前配置端口没有 cfg_update，因此用外部值与已采样值的差异唤醒。
    assign cfg_change_req =
        (cfg_in_q_min_cell        != cfg_q_min_cell[0])        |
        (cfg_in_q_max_cell        != cfg_q_max_cell[0])        |
        (cfg_in_port_max          != cfg_port_max[0])          |
        (cfg_in_global_max        != cfg_global_max)           |
        (cfg_in_pause_en          != cfg_pause_en)             |
        (cfg_in_port_pause_xoff   != cfg_port_pause_xoff[0])   |
        (cfg_in_port_pause_xon    != cfg_port_pause_xon[0])    |
        (cfg_in_global_pause_xoff != cfg_global_pause_xoff)    |
        (cfg_in_global_pause_xon  != cfg_global_pause_xon)     |
        (cfg_in_pfc_en            != cfg_pfc_en)               |
        (cfg_in_pfc_xoff          != cfg_pfc_xoff[0][0])       |
        (cfg_in_pfc_xon           != cfg_pfc_xon[0][0])        |
        (cfg_in_aging_en          != cfg_aging_en)             |
        (cfg_in_aging_timeout     != cfg_aging_timeout)        |
        (cfg_in_age_force_all     != cfg_age_force[0]);

    assign wake_req = init_start           |
                      enq_req              |
                      deq_req              |
                      recycle_req          |
                      age_flush_req        |
    ...
    // 队列占用本身不再阻止休眠；只等待已受理的操作和内部脉冲排空。
    assign core_work_pending = wake_req          |
                               lle_busy          |
                               init_build_req    |
                               clr_ptr_cnt       |
                               age_flush_req     |
    ...
    // 两拍空闲确认保证单拍 valid/ack、SRAM 返回和计数更新全部落地。
    always_ff @(posedge smmu_core_clk or negedge rst_core_n) begin
        if (!rst_core_n) begin
            core_idle_q <= 1'b0;
            idle_qual_q <= 1'b0;
        end
        else if (core_work_pending) begin
            core_idle_q <= 1'b0;
            idle_qual_q <= 1'b0;
        end
        else if (!core_idle_q) begin
            if (idle_qual_q) begin
                core_idle_q <= 1'b1;
            end
            else begin
                idle_qual_q <= 1'b1;
    ...
    assign clock_req = ~core_idle_q | wake_req;

    generate
        if (CLK_GATING_EN != 0) begin : GEN_SMMU_RTL_CLK_GATE
            smmu_clock_gate_rtl u_smmu_clock_gate (
                .clk_in  (clk_core),
                .rst_n   (rst_core_n),
                .enable  (clock_req),
                .clk_out (smmu_core_clk)
            );
        end
        else begin : GEN_SMMU_CLK_BYPASS
            assign smmu_core_clk = clk_core;
        end
    endgenerate
    ...
    dequeue_ctrl #(
        ...
        .deq_egress_port    (deq_egress_port),
        ...
        .lle_qhead_next_pkt_tail (lle_qhead_next_pkt_tail),
    ...
    recycle_ctrl #(
        ...
        .recycle_egress_port    (recycle_egress_port),
    ...
    lle #(
        ...
        .evt_free_cell_num     (evt_free_cell_num),
        // ★ 老化冲刷 ↔ aging_ctrl
    ...
    aging_ctrl #(
        ...
        // 常开老化岛: 核心休眠时仍按真实 cycle 计时，超时请求反向唤醒核心。
        .clk_core          (clk_core),
        .rst_core_n        (rst_core_n),
    ...
    occupancy_pool_mgr #(
        ...
        .occ_free_cell_num     (evt_free_cell_num),
        // 流控 / max 输出
        .pause_req             (pause_req),
        .pfc_req               (pfc_req),
...
//============================================================================
// Module      : smmu_clock_gate_rtl
// Description : 纯 RTL 低电平透明门控时钟。enable 只在 clk_in 低电平时被锁存，
//               因此高电平期间不会改变 clk_out，也不会截断有效高脉冲。
//               本模块不例化任何工艺时钟门控 cell。
//============================================================================
module smmu_clock_gate_rtl (
    input  logic clk_in,
...
`ifdef SIM_BEHAVIOR_SRAM
    // 门控时钟不得产生脱离父时钟边沿的额外脉冲。
    always @(posedge clk_out) begin
        if (rst_n && (clk_in !== 1'b1))
            $error("[smmu_clock_gate_rtl] unexpected gated-clock rising edge");
    end
    always @(negedge clk_out) begin
        if (rst_n && (clk_in !== 1'b0))
            $error("[smmu_clock_gate_rtl] truncated/extra gated-clock falling edge");
    end
`endif
