...
module smmu_tb;
    ...
    localparam int MAX_MC_FRAMES = 4;
    localparam int CLK_GATING_EN = 1;
    ...
    localparam int QID_W      = (QUEUE_NUM > 1) ? $clog2(QUEUE_NUM) : 1; // 6
    localparam int PORT_W     = (PORT_NUM  > 1) ? $clog2(PORT_NUM)  : 1; // 2
    localparam int TC_W       = (TC_NUM    > 1) ? $clog2(TC_NUM)    : 1; // 3
    ...
    smmu #(
        ...
        .deq_pkt_tail_next  (deq_pkt_tail_next),
        // 回收 (统一还链接口)
    ...
    // 等待核心进入休眠，并确认指定数量的父时钟周期内一直未被唤醒。
    task automatic check_core_sleep_window(input string case_id,
                                           input string desc,
                                           input int cycles);
        logic sleep_stable;
        wait(u_dut.core_idle_q == 1'b1);
        sleep_stable = 1'b1;
        repeat (cycles) begin
            @(negedge clk_core);
            if ((u_dut.core_idle_q !== 1'b1) ||
                (u_dut.smmu_core_clk !== 1'b0)) begin
                sleep_stable = 1'b0;
            end
        end
        check(case_id, desc, sleep_stable);
    endtask
    ...
    task automatic test_UNI_ENQ_006();
        ...
        enq_req <= 0; enq_eof <= 0;
        @(posedge clk_core);
        // Check: frame should be dropped and frame_drop_q should clear at EOF.
        check("UNI_ENQ-006b", "frame_drop_q cleared at EOF", `ENQ_PATH.frame_drop_q == 0);
    ...
    task automatic test_MC_ENQ_001();
        ...
        wait_clks(2);
        check("MC_ENQ-001a", "mc_valid[0]=1", `LLE_PATH.mc_valid_q[0] == 1);
        check("MC_ENQ-001b", "mc_dst_bitmap[0]=4'b1010", `LLE_PATH.mc_dst_bitmap_q[0] == 4'b1010);
        check("MC_ENQ-001c", "mc_ncell[0]=2", `LLE_PATH.mc_ncell_q[0] == 2);
        // carry_qid[1] = 1*8+3 = 11
        check("MC_ENQ-001d", "mc_carry_qid[0][1]=11", `LLE_PATH.mc_carry_qid_q[0][1] == 11);
        // carry_qid[3] = 3*8+3 = 27
        check("MC_ENQ-001e", "mc_carry_qid[0][3]=27", `LLE_PATH.mc_carry_qid_q[0][3] == 27);
        // MC_QID cell count
    ...
    task automatic test_MC_ENQ_005();
        ...
        wait_clks(2);
        check("MC_ENQ-005a", "mc_valid[0]=1", `LLE_PATH.mc_valid_q[0] == 1);
        // Second multicast should occupy slot 1 rather than being dropped.
        @(posedge clk_core);
        enq_req <= 1; enq_is_mcast <= 1; enq_mcast_bitmap <= 4'b0010;
        enq_queue_id <= 1; enq_egress_port <= 0;
        enq_sof <= 1; enq_eof <= 1; enq_cell_num <= 1;
        @(negedge clk_core);
        check("MC_ENQ-005b", "enq_predict_drop=0 (free slot remains)", enq_predict_drop == 0);
        @(posedge clk_core);
        enq_req <= 0; enq_sof <= 0; enq_eof <= 0; enq_is_mcast <= 0;
        @(posedge clk_core);
        check("MC_ENQ-005c", "alloc_drop_ind=0", alloc_drop_ind == 0);
        check("MC_ENQ-005d", "mc_valid[1]=1", `LLE_PATH.mc_valid_q[1] == 1);

        // Fill slot 2 and slot 3.
        for (int slot_idx = 2; slot_idx < 4; slot_idx++) begin
            @(posedge clk_core);
            enq_req <= 1; enq_is_mcast <= 1; enq_mcast_bitmap <= (4'b0001 << slot_idx);
            enq_queue_id <= TC_W'(slot_idx); enq_sof <= 1; enq_eof <= 1; enq_cell_num <= 1;
            @(negedge clk_core);
            check($sformatf("MC_ENQ-005-fill-%0d", slot_idx),
                  "free multicast slot accepted", enq_predict_drop == 0);
            @(posedge clk_core);
            enq_req <= 0; enq_sof <= 0; enq_eof <= 0; enq_is_mcast <= 0;
            @(posedge clk_core);
            check($sformatf("MC_ENQ-005-valid-%0d", slot_idx),
                  "slot valid", `LLE_PATH.mc_valid_q[slot_idx] == 1);
        end

        // The fifth concurrent frame must be dropped because all four slots are occupied.
        @(posedge clk_core);
        enq_req <= 1; enq_is_mcast <= 1; enq_mcast_bitmap <= 4'b1111;
        enq_queue_id <= 0; enq_sof <= 1; enq_eof <= 1; enq_cell_num <= 1;
        @(negedge clk_core);
        check("MC_ENQ-005e", "fifth frame predict-drop", enq_predict_drop == 1);
        @(posedge clk_core);
        enq_req <= 0; enq_sof <= 0; enq_eof <= 0; enq_is_mcast <= 0;
        @(posedge clk_core);
        check("MC_ENQ-005f", "fifth frame drop indication", alloc_drop_ind == 1);
    endtask
    ...
    task automatic test_DROP_005();
        ...
        @(posedge clk_core);
        check("DROP-005a", "alloc_drop_ind=1 at EOF", alloc_drop_ind == 1);
    ...
    task automatic test_RCY_004();
        ...
        wait_clks(2);
        // ref_cnt[0] 初值应为 N=2
        check("RCY-004a", "mc_ref_cnt[0][0]=2 (N=popcount)", `LLE_PATH.mc_ref_cnt_q[0][0] == 2);
        ...
        check("RCY-004c", "mc_ref_cnt[0][0]=1 (递减)", `LLE_PATH.mc_ref_cnt_q[0][0] == 1);
        check("RCY-004d", "mc_valid[0]=1 (未到0不释放)", `LLE_PATH.mc_valid_q[0] == 1);
    endtask
    ...
    task automatic test_MC_ENQ_002();
        ...
        check("MC_ENQ-002a", "mc_pend_uni[0][0]=0", `LLE_PATH.mc_pend_uni_q[0][0] == 0);
        ...
        wait_clks(1);
        check("MC_ENQ-002e", "mc_rd_done[0][0]=1", `LLE_PATH.mc_rd_done_q[0][0] == 1);
    endtask
    ...
    task automatic test_MC_ENQ_003();
        ...
        check("MC_ENQ-003a", "mc_pend_uni[0][0]=1", `LLE_PATH.mc_pend_uni_q[0][0] == 1);
    ...
    task automatic test_MC_ENQ_004();
        ...
        wait_clks(10);
        check("MC_ENQ-004a", "mc_valid[0]=0 (slot released)", `LLE_PATH.mc_valid_q[0] == 0);
    ...
    task automatic test_DROP_006();
        ...
        check("DROP-006b", "single-cell drop indicated by alloc_drop_ind", alloc_drop_ind == 1);
    ...
    task automatic test_RCY_005();
        ...
        recycle_req <= 0; recycle_is_mcast <= 0;
        wait_clks(15);
        check("RCY-005a", "mc_valid[0]=0 (还满N=2次后释放)", `LLE_PATH.mc_valid_q[0] == 0);
    ...
    task automatic test_AGE_005();
        ...
        check("AGE-005d", "整链一次挂回后 LLE free_cnt 恢复", `LLE_PATH.free_cnt_q == CELL_NUM);
        check("AGE-005e", "整链批量事件后 occupancy 守恒",
              (`OCC_PATH.free_count_q == CELL_NUM) && (`OCC_PATH.global_used_q == 0) &&
              (`OCC_PATH.q_cell_cnt_q[0] == 0));
        check("AGE-005f", "原老化链 tail 成为新的 free_tail", `LLE_PATH.free_tail_q == 2);
    endtask
    ...
    task automatic test_CORNER_003();
        ...
        enq_req <= 0; enq_eof <= 0; enq_is_mcast <= 0;
        wait_clks(3);
        check("CORNER-003a", "mc_ncell[0]=8", `LLE_PATH.mc_ncell_q[0] == 8);
        check("CORNER-003b", "mc_valid[0]=1", `LLE_PATH.mc_valid_q[0] == 1);
    endtask
    ...
    task automatic test_CORNER_004();
        ...
        for (int p = 0; p < PORT_NUM; p++) begin
            for (int t = 0; t < TC_NUM; t++) begin
                enqueue_cell(p[PORT_W-1:0], t[TC_W-1:0], 1, 1, 1);
                wait_clks(0);
            end
        end
        wait_clks(5);
    ...
    task automatic test_AGE_006();
        ...
        enq_req <= 0; enq_sof <= 0; enq_eof <= 0; enq_is_mcast <= 0;
        wait_clks(2);
        check("AGE-006a", "mc_valid[0]=1 before flush", `LLE_PATH.mc_valid_q[0] == 1);
        ...
        check("AGE-006c", "mc_valid[0]=0 after MC flush", `LLE_PATH.mc_valid_q[0] == 0);
    endtask
...
//========================================================================
    ...
    task automatic test_MC_ENQ_008();
        ...
        enq_req <= 0; enq_eof <= 0; enq_is_mcast <= 0;
        wait_clks(2);
        check("MC_ENQ-008a", "mc_pend_uni[0][0]=0", `LLE_PATH.mc_pend_uni_q[0][0] == 0);
        check("MC_ENQ-008b", "mc_pend_uni[0][1]=1", `LLE_PATH.mc_pend_uni_q[0][1] == 1);
        check("MC_ENQ-008c", "mc_pend_uni[0][2]=2", `LLE_PATH.mc_pend_uni_q[0][2] == 2);
        check("MC_ENQ-008d", "mc_pend_uni[0][3]=3", `LLE_PATH.mc_pend_uni_q[0][3] == 3);
        @(posedge clk_core);
        deq_req <= 1; deq_queue_id <= 0; deq_backpressure <= 0;
        @(posedge clk_core);
        deq_req <= 0;
    ...
    task automatic test_MC_ENQ_009();
        ...
        enq_req <= 0; enq_sof <= 0; enq_eof <= 0; enq_is_mcast <= 0;
        wait_clks(2);
        check("MC_ENQ-009b", "mc_pend_uni[0][0]=快照backlog=1", `LLE_PATH.mc_pend_uni_q[0][0] == 1);
    endtask
    ...
    task automatic test_MC_ENQ_010();
        ...
        deq_req <= 0;
        wait_clks(2);
        check("MC_ENQ-010a", "mc_rd_done[0][0]=1 (port0读完)", `LLE_PATH.mc_rd_done_q[0][0] == 1);
        check("MC_ENQ-010b", "mc_rd_done[0][1]=0 (port1未读)", `LLE_PATH.mc_rd_done_q[0][1] == 0);
        check("MC_ENQ-010c", "mc_valid[0]=1 (未全read不释放)", `LLE_PATH.mc_valid_q[0] == 1);
    endtask
    ...
    task automatic test_MC_ENQ_011();
        ...
        enq_req <= 0; enq_sof <= 0; enq_eof <= 0; enq_is_mcast <= 0;
        wait_clks(2);
        check("MC_ENQ-011a", "mc_ncell[0]=1", `LLE_PATH.mc_ncell_q[0] == 1);
        @(posedge clk_core);
        deq_req <= 1; deq_queue_id <= 0; deq_backpressure <= 0;
        @(posedge clk_core);
        deq_req <= 0;
    ...
    task automatic test_MC_ENQ_012();
        ...
        check("MC_ENQ-012a", "mc_ncell[0]=8", `LLE_PATH.mc_ncell_q[0] == 8);
        check("MC_ENQ-012b", "mc_dst_bitmap[0]=4'b1101", `LLE_PATH.mc_dst_bitmap_q[0] == 4'b1101);
        ...
        check("MC_ENQ-012c", "mc_rd_done[0][0]=1", `LLE_PATH.mc_rd_done_q[0][0] == 1);
        check("MC_ENQ-012d", "mc_rd_done[0][2]=1", `LLE_PATH.mc_rd_done_q[0][2] == 1);
        check("MC_ENQ-012e", "mc_rd_done[0][3]=1", `LLE_PATH.mc_rd_done_q[0][3] == 1);

        for (int port_sel = 0; port_sel < 3; port_sel++) begin
            for (int cell_idx = 0; cell_idx < 8; cell_idx++) begin
                @(posedge clk_core);
                recycle_req <= 1;
                recycle_cell_addr <= `LLE_PATH.mc_cells_q[0][cell_idx];
        ...
        check("MC_ENQ-012f", "mc_valid[0]=0 after recycle", `LLE_PATH.mc_valid_q[0] == 0);
    ...
    task automatic test_RCY_007();
        ...
        for (int i = 0; i < 3; i++) begin
            ...
            recycle_req <= 1; recycle_cell_addr <= i[ADDR_W-1:0]; recycle_queue_id <= 0;
            @(negedge clk_core);
            if ((`LLE_PATH.ext_free_push + `LLE_PATH.mc_rel_push) > 1)
                onehot_ok = 0;
        end
    ...
    task automatic test_RCY_008();
        ...
        recycle_req <= 0; recycle_is_mcast <= 0;
        wait_clks(15);
        check("RCY-008a", "mc_valid[0]=0 (最终释放)", `LLE_PATH.mc_valid_q[0] == 0);
        check("RCY-008b", "守恒成立",
              (`OCC_PATH.free_count_q + `OCC_PATH.global_used_q) == CELL_NUM);
    endtask
    ...
    task automatic test_AGE_011();
        ...
        cfg_in_age_force_all <= 0;
        wait_clks(10);
        check("AGE-011a", "mc_valid[0]=0 (冲刷清槽)", `LLE_PATH.mc_valid_q[0] == 0);
    ...
    task automatic test_AGE_012();
        ...
        check("AGE-012b", "MC 槽已释放", `LLE_PATH.mc_valid_q[0] == 0);
    endtask
    ...
    task automatic test_AGE_014();
        ...
        check("AGE-014b", "port0/1已读完, port2/3未读",
              (`LLE_PATH.mc_rd_done_q[0][0] == 1) &&
              (`LLE_PATH.mc_rd_done_q[0][1] == 1) &&
              (`LLE_PATH.mc_rd_done_q[0][2] == 0) &&
              (`LLE_PATH.mc_rd_done_q[0][3] == 0));
        ...
        check("AGE-014e", "mc_valid[0]=0 after MC aging", `LLE_PATH.mc_valid_q[0] == 0);
    ...
    task automatic test_LLE_011();
        logic [QID_W-1:0] qid_p1_q2;
        logic [QID_W-1:0] qid_p0_q1;
        logic deq_valid_before_enq_done;
        logic q10_deq_done;
        logic q1_deq_done;
        logic enq_done_this_cycle;
        int enq_idx;
        int guard;

        $display("\n===== LLE-011: enq未结束时发起另一队列deq =====");
        reset_dut();
        do_init();
        qid_p1_q2 = QID_W'(1*TC_NUM + 2);
        qid_p0_q1 = QID_W'(0*TC_NUM + 1);

        // port1/queue2 先入一个完整 6-cell packet。
        enqueue_frame(1, 2, 6);
        wait_clks(3);
        check("LLE-011a", "port1/q2 已有一个6cell完整pkt",
              (`LLE_PATH.q_cell_cnt_q[qid_p1_q2] == 6) &&
              (`LLE_PATH.q_uni_pkt_backlog_q[qid_p1_q2] == 1));

        deq_valid_before_enq_done = 1'b0;

        // 隔 3 cycle 后开始 port0/queue1 的 5-cell packet。
        // 在第 3 个 cell 开始发起 port1/queue2 的 dequeue，并保持请求。
        wait_clks(3);
        enq_idx = 0;
        guard = 0;
        while (enq_idx < 5 && guard < 40) begin
            @(posedge clk_core);
            enq_req <= 1;
            enq_egress_port <= 0;
            enq_queue_id <= 1;
            ...
            if (enq_idx >= 2) begin
                deq_req <= 1;
                deq_queue_id <= qid_p1_q2;
                deq_backpressure <= 0;
            end

            @(negedge clk_core);
            enq_done_this_cycle = (enq_idx == 4) && enq_ready;
            if (!enq_done_this_cycle && deq_cell_valid)
                deq_valid_before_enq_done = 1'b1;
            if (enq_ready)
                enq_idx++;
            guard++;
        end
        ...
        // EOF 所在 cell 已被采样前，不应提前返回 port1/q2 的出队结果。
        @(negedge clk_core);
        check("LLE-011b", "port0/q1 enq完成前 port1/q2 deq不应返回valid",
              deq_valid_before_enq_done == 1'b0);

        deq_req <= 0;
        wait_clks(5);
        check("LLE-011c", "port0/q1 5cell完整pkt入队完成",
              (`LLE_PATH.q_cell_cnt_q[qid_p0_q1] == 5) &&
              (`LLE_PATH.q_uni_pkt_backlog_q[qid_p0_q1] == 1));
        ...
        deq_req <= 0;
        wait_clks(4);
        q10_deq_done = (`LLE_PATH.q_cell_cnt_q[qid_p1_q2] == 0) &&
                       (`LLE_PATH.q_uni_pkt_backlog_q[qid_p1_q2] == 0);
        check("LLE-011d", "port1/q2 packet完整出队完成", q10_deq_done == 1'b1);

        for (int c = 0; c < 5; c++) begin
            @(posedge clk_core);
            deq_req <= 1;
            deq_queue_id <= qid_p0_q1;
            deq_backpressure <= 0;
        end
        @(posedge clk_core);
        deq_req <= 0;
        wait_clks(4);
        q1_deq_done = (`LLE_PATH.q_cell_cnt_q[qid_p0_q1] == 0) &&
                      (`LLE_PATH.q_uni_pkt_backlog_q[qid_p0_q1] == 0);
        check("LLE-011e", "port0/q1 packet随后完整出队完成", q1_deq_done == 1'b1);
    endtask

    task automatic test_LLE_012();
        logic [QID_W-1:0] qid_p2_q3;
        int pkt_len [3];
        int remain_cells;
        logic enq_ok;
        logic pkt_deq_ok;

        $display("\n===== LLE-012: 同队列多pkt入队和逐pkt出队 =====");
        reset_dut();
        do_init();

        qid_p2_q3 = QID_W'(2*TC_NUM + 3);
        pkt_len[0] = 5;
        pkt_len[1] = 6;
        pkt_len[2] = 7;

        for (int p = 0; p < 3; p++) begin
            enqueue_frame(2, 3, pkt_len[p]);
            wait_clks(1);
        end
        wait_clks(4);

        enq_ok = (`LLE_PATH.q_cell_cnt_q[qid_p2_q3] == (pkt_len[0] + pkt_len[1] + pkt_len[2])) &&
                 (`LLE_PATH.q_uni_pkt_backlog_q[qid_p2_q3] == 3);
        check("LLE-012a", "port2/q3 连续入3个pkt(5/6/7cell), pkt计数=3", enq_ok == 1'b1);

        remain_cells = pkt_len[0] + pkt_len[1] + pkt_len[2];
        for (int p = 0; p < 3; p++) begin
            for (int c = 0; c < pkt_len[p]; c++) begin
                @(posedge clk_core);
                deq_req <= 1;
                deq_queue_id <= qid_p2_q3;
                deq_backpressure <= 0;
            end
            @(posedge clk_core);
            deq_req <= 0;
            ...
            remain_cells -= pkt_len[p];
            pkt_deq_ok = (`LLE_PATH.q_cell_cnt_q[qid_p2_q3] == remain_cells) &&
                         (`LLE_PATH.q_uni_pkt_backlog_q[qid_p2_q3] == (2 - p));
            case (p)
                0: check("LLE-012b", "第1个5cell pkt出队后pkt计数=2", pkt_deq_ok == 1'b1);
                1: check("LLE-012c", "第2个6cell pkt出队后pkt计数=1", pkt_deq_ok == 1'b1);
                default: check("LLE-012d", "第3个7cell pkt出队后cell/pkt计数清0", pkt_deq_ok == 1'b1);
            endcase
        end
    endtask

    task automatic test_LLE_013();
        logic [QID_W-1:0] qid_p0_q0;
        logic [QID_W-1:0] qid_p0_q1;
        logic bp_pause_ok;
        logic deq_valid_before_enq_done;
        logic enq_done_this_cycle;
        ...
        $display("\n===== LLE-013: deq背压暂停后enq锁包优先 =====");
        reset_dut();
        do_init();

        qid_p0_q0 = QID_W'(0*TC_NUM + 0);
        qid_p0_q1 = QID_W'(0*TC_NUM + 1);

        // 先入 port0/q0 一个 6-cell packet。
        enqueue_frame(0, 0, 6);
        wait_clks(3);
        check("LLE-013a", "port0/q0 已有一个6cell完整pkt",
              (`LLE_PATH.q_cell_cnt_q[qid_p0_q0] == 6) &&
              (`LLE_PATH.q_uni_pkt_backlog_q[qid_p0_q0] == 1));

        // 先出队 4 个 cell, 剩余 2 个 cell 且 pkt backlog 仍为 1。
        for (int c = 0; c < 4; c++) begin
            @(posedge clk_core);
            deq_req <= 1;
            deq_queue_id <= qid_p0_q0;
            deq_backpressure <= 0;
        end
        @(posedge clk_core);
        deq_req <= 0;
        wait_clks(4);
        check("LLE-013b", "port0/q0 deq4后剩2cell且pkt计数仍为1",
              (`LLE_PATH.q_cell_cnt_q[qid_p0_q0] == 2) &&
              (`LLE_PATH.q_uni_pkt_backlog_q[qid_p0_q0] == 1));

        // 拉高 port0 背压并持续发 deq 请求, deq 应暂停。
        bp_pause_ok = 1'b1;
        ...
        check("LLE-013c", "port0背压拉高时deq暂停", bp_pause_ok == 1'b1);

        // 背压保持期间来一个新的 port0/q1 6-cell enq。入到一半后释放背压,
        // deq_req 仍保持, 但 enq packet 应锁住直到 EOF, 不被 deq 打断。
        deq_valid_before_enq_done = 1'b0;
        enq_idx = 0;
        guard = 0;
        while (enq_idx < 6 && guard < 50) begin
            @(posedge clk_core);
            enq_req <= 1;
            enq_egress_port <= 0;
            enq_queue_id <= 1;
            ...
            deq_req <= 1;
            deq_queue_id <= qid_p0_q0;
            deq_backpressure <= (enq_idx < 3) ? 4'b0001 : 4'b0000;

            @(negedge clk_core);
            enq_done_this_cycle = (enq_idx == 5) && enq_ready;
            if ((enq_idx >= 3) && !enq_done_this_cycle && deq_cell_valid)
                deq_valid_before_enq_done = 1'b1;
            if (enq_ready)
                enq_idx++;
            guard++;
        ...
        @(negedge clk_core);
        check("LLE-013d", "背压释放后enq完成前deq不返回valid",
              deq_valid_before_enq_done == 1'b0);

        wait_clks(4);
        check("LLE-013e", "port0/q1 enq完成且pkt计数=1",
              (`LLE_PATH.q_cell_cnt_q[qid_p0_q1] == 6) &&
              (`LLE_PATH.q_uni_pkt_backlog_q[qid_p0_q1] == 1));

        // enq 结束后继续 deq q0, 应把剩余 2 个 cell 出完。
        for (int c = 0; c < 4; c++) begin
            @(posedge clk_core);
            deq_req <= 1;
            deq_queue_id <= qid_p0_q0;
        ...
        final_ok = (`LLE_PATH.q_cell_cnt_q[qid_p0_q0] == 0) &&
                   (`LLE_PATH.q_uni_pkt_backlog_q[qid_p0_q0] == 0) &&
                   (`LLE_PATH.q_cell_cnt_q[qid_p0_q1] == 6) &&
                   (`LLE_PATH.q_uni_pkt_backlog_q[qid_p0_q1] == 1);
        check("LLE-013f", "enq结束后q0剩余cell继续deq完成,q1保持完整pkt", final_ok == 1'b1);
    endtask

    task automatic test_LLE_014();
        logic [QID_W-1:0] qid_p1_q4;
        logic no_valid_during_pause;
        logic resume_valid_seen;
        logic final_ok;

        $display("\n===== LLE-014: deq请求暂停后继续同一pkt =====");
        reset_dut();
        do_init();

        qid_p1_q4 = QID_W'(1*TC_NUM + 4);

        // 先入 port1/q4 一个 8-cell packet。
        enqueue_frame(1, 4, 8);
        wait_clks(3);
        check("LLE-014a", "port1/q4 已有一个8cell完整pkt",
              (`LLE_PATH.q_cell_cnt_q[qid_p1_q4] == 8) &&
              (`LLE_PATH.q_uni_pkt_backlog_q[qid_p1_q4] == 1));

        // 请求前 5 个 cell。
        for (int c = 0; c < 5; c++) begin
            @(posedge clk_core);
            deq_req <= 1;
            deq_queue_id <= qid_p1_q4;
            deq_backpressure <= 0;
        end
        @(posedge clk_core);
        deq_req <= 0;
        wait_clks(4);
        check("LLE-014b", "deq5后剩3cell且pkt计数仍为1",
              (`LLE_PATH.q_cell_cnt_q[qid_p1_q4] == 3) &&
              (`LLE_PATH.q_uni_pkt_backlog_q[qid_p1_q4] == 1));

        // 暂停 deq_req 几个 cycle, 队列状态应保持。
        no_valid_during_pause = 1'b1;
        for (int c = 0; c < 4; c++) begin
            @(posedge clk_core);
            deq_req <= 0;
            deq_backpressure <= 0;
            @(negedge clk_core);
            if (deq_cell_valid)
                no_valid_during_pause = 1'b0;
        end
        check("LLE-014c", "deq_req暂停期间无新的deq valid", no_valid_during_pause == 1'b1);
        check("LLE-014d", "暂停后仍剩3cell且pkt计数仍为1",
              (`LLE_PATH.q_cell_cnt_q[qid_p1_q4] == 3) &&
              (`LLE_PATH.q_uni_pkt_backlog_q[qid_p1_q4] == 1));

        // 继续请求, 应从上次位置继续直到 pkt tail 出队。
        resume_valid_seen = 1'b0;
        for (int c = 0; c < 5; c++) begin
            @(posedge clk_core);
            deq_req <= 1;
            deq_queue_id <= qid_p1_q4;
            ...
            @(negedge clk_core);
            if (deq_cell_valid)
                resume_valid_seen = 1'b1;
        end
        @(posedge clk_core);
        deq_req <= 0;
        wait_clks(4);

        final_ok = (`LLE_PATH.q_cell_cnt_q[qid_p1_q4] == 0) &&
                   (`LLE_PATH.q_uni_pkt_backlog_q[qid_p1_q4] == 0);
        check("LLE-014e", "恢复请求后继续看到deq valid", resume_valid_seen == 1'b1);
        check("LLE-014f", "恢复后同一8cell pkt完整出队结束", final_ok == 1'b1);
    endtask

    task automatic test_LLE_015();
        logic [QID_W-1:0] qid_p3_q5;
        int pkt_len [3];
        int total_cells;
        logic enq_ok;
        logic gap_ok;
        ...
        $display("\n===== LLE-015: 同队列三pkt, deq先断后连 =====");
        reset_dut();
        do_init();

        qid_p3_q5 = QID_W'(3*TC_NUM + 5);
        pkt_len[0] = 6;
        pkt_len[1] = 8;
        pkt_len[2] = 7;
        total_cells = pkt_len[0] + pkt_len[1] + pkt_len[2];

        for (int p = 0; p < 3; p++) begin
            enqueue_frame(3, 5, pkt_len[p]);
            wait_clks(1);
        end
        wait_clks(4);

        enq_ok = (`LLE_PATH.q_cell_cnt_q[qid_p3_q5] == total_cells) &&
                 (`LLE_PATH.q_uni_pkt_backlog_q[qid_p3_q5] == 3);
        check("LLE-015a", "port3/q5 入3个不同长度pkt(6/8/7)", enq_ok == 1'b1);

        // 第一个 packet 单独 deq 完, 然后停几拍。
        for (int c = 0; c < pkt_len[0]; c++) begin
            @(posedge clk_core);
            deq_req <= 1;
            deq_queue_id <= qid_p3_q5;
            deq_backpressure <= 0;
        end
        @(posedge clk_core);
        deq_req <= 0;
        ...
        gap_ok = (`LLE_PATH.q_cell_cnt_q[qid_p3_q5] == (total_cells - pkt_len[0])) &&
                 (`LLE_PATH.q_uni_pkt_backlog_q[qid_p3_q5] == 2);
        check("LLE-015b", "第1个pkt出队后停顿, 剩2个pkt", gap_ok == 1'b1);

        // 第二、第三个 packet 连续 deq, 中间 deq_req 不断。
        for (int c = 0; c < (pkt_len[1] + pkt_len[2]); c++) begin
            @(posedge clk_core);
            deq_req <= 1;
            deq_queue_id <= qid_p3_q5;
            deq_backpressure <= 0;
        ...
        final_ok = (`LLE_PATH.q_cell_cnt_q[qid_p3_q5] == 0) &&
                   (`LLE_PATH.q_uni_pkt_backlog_q[qid_p3_q5] == 0);
        check("LLE-015c", "第2和第3个pkt连续出队后清空", final_ok == 1'b1);
    endtask
    ...
    //------------------------------------------------------------------------
    // 低功耗门控时钟: 休眠、无额外拍唤醒、背靠背无气泡
    //------------------------------------------------------------------------
    task automatic test_LP_001();
        $display("\n===== LP-001: 空闲后核心时钟停在低电平 =====");
        reset_dut();
        do_init();
        wait_clks(6);
        check("LP-001a", "core_idle asserted", u_dut.core_idle_q == 1'b1);
        check("LP-001b", "smmu_core_clk held low", u_dut.smmu_core_clk == 1'b0);
    endtask

    task automatic test_LP_002();
        $display("\n===== LP-002: 休眠唤醒不增加入队拍数 =====");
        reset_dut();
        do_init();
        wait(u_dut.core_idle_q == 1'b1);

        // 请求在原时钟低电平到达，紧接着的上升沿应直接被 SMMU 采样。
        @(negedge clk_core);
        enq_req <= 1'b1;
        enq_egress_port <= '0;
        enq_queue_id <= '0;
        enq_cell_num <= PKT_CELL_W'(1);
        enq_is_mcast <= 1'b0;
        enq_sof <= 1'b1;
        enq_eof <= 1'b1;
        @(posedge clk_core);
        @(negedge clk_core);
        check("LP-002a", "wake request accepted on first parent edge", alloc_valid == 1'b1);
        check("LP-002b", "no drop after wake", alloc_drop_ind == 1'b0);
        enq_req <= 1'b0;
        enq_sof <= 1'b0;
        enq_eof <= 1'b0;
    endtask

    task automatic test_LP_003();
        logic valid_all;
        $display("\n===== LP-003: 从休眠唤醒后背靠背入队无气泡 =====");
        reset_dut();
        do_init();
        wait(u_dut.core_idle_q == 1'b1);
        ...
        enq_egress_port <= '0;
        enq_queue_id <= '0;
        enq_cell_num <= PKT_CELL_W'(3);
        enq_is_mcast <= 1'b0;
        enq_sof <= 1'b1;
        enq_eof <= 1'b0;

        for (int cell_idx = 0; cell_idx < 3; cell_idx++) begin
            @(posedge clk_core);
            @(negedge clk_core);
            if (!alloc_valid) valid_all = 1'b0;
            enq_sof <= 1'b0;
            enq_eof <= (cell_idx == 1);
        end
        enq_req <= 1'b0;
        enq_eof <= 1'b0;
        check("LP-003a", "three consecutive cells accepted", valid_all == 1'b1);
    endtask

    task automatic test_LP_004();
        $display("\n===== LP-004: 队列非空但无请求时休眠 =====");
        reset_dut();
        do_init();
        enqueue_frame(0, 0, 1);
        wait_clks(8);
        check("LP-004a", "queue remains occupied", u_dut.q_occupied_vec[0] == 1'b1);
        check("LP-004b", "non-empty idle core enters sleep", u_dut.core_idle_q == 1'b1);
        check("LP-004c", "non-empty idle core clock held low", u_dut.smmu_core_clk == 1'b0);
    endtask

    task automatic test_LP_005();
        logic wake_seen;
        logic flush_done_seen;

        $display("\n===== LP-005: 休眠期间老化超时自动唤醒冲刷 =====");
        reset_dut();
        do_init();
        cfg_in_aging_en <= 1'b1;
        cfg_in_aging_timeout <= 24'd40;
        wait_clks(3);
        enqueue_frame(0, 0, 1);

        // enqueue_frame 在 posedge 用 NBA 撤销 enq_req；此时 core_idle_q
        // 可能仍是该边沿前的旧值 1。先等待入队状态落地和核心
        // 确实进入工作态，再等待本次无请求后的新休眠边界。
        wait(u_dut.q_occupied_vec[0] == 1'b1);
        wait(u_dut.core_idle_q == 1'b0);
        wait(u_dut.core_idle_q == 1'b1);
        @(negedge clk_core);
        check("LP-005a", "occupied queue sleeps before timeout", u_dut.q_occupied_vec[0] == 1'b1);

        wake_seen = 1'b0;
        flush_done_seen = 1'b0;
        for (int cycle_idx = 0; cycle_idx < 100; cycle_idx++) begin
            @(negedge clk_core);
            if (!u_dut.core_idle_q)
                wake_seen = 1'b1;
            if (u_dut.age_flush_done)
                flush_done_seen = 1'b1;
        end
        check("LP-005b", "aging timeout wakes gated core", wake_seen == 1'b1);
        check("LP-005c", "aging flush completes after wake", flush_done_seen == 1'b1);
        check("LP-005d", "aged queue becomes empty", u_dut.q_occupied_vec[0] == 1'b0);
    endtask

    task automatic test_LP_006();
        logic [ADDR_W-1:0] alloc_addr_q [6];
        logic [ADDR_W-1:0] deq_addr_q [6];
        logic alloc_ok;
        logic deq_ok;
        logic recycle_ok;

        $display("\n===== LP-006: 建链/挂链/走链/还链后持续休眠 =====");
        reset_dut();
        do_init();

        // 1) 建链: 初始化应建成完整 free 链。
        check("LP-006a", "free chain build completes",
              (init_done == 1'b1) && (`LLE_PATH.free_cnt_q == CNT_W'(CELL_NUM)));
        check_core_sleep_window("LP-006b", "sleep remains stable for 30 cycles after build", 30);

        // 2) 挂链: 从休眠唤醒，背靠背挂入一个 6-cell 单播帧。
        alloc_ok = 1'b1;
        @(negedge clk_core);
        enq_req <= 1'b1;
        enq_egress_port <= PORT_W'(0);
        enq_queue_id <= TC_W'(0);
        enq_cell_num <= PKT_CELL_W'(6);
        enq_is_mcast <= 1'b0;
        enq_mcast_bitmap <= '0;
        enq_sof <= 1'b1;
        enq_eof <= 1'b0;
        for (int cell_idx = 0; cell_idx < 6; cell_idx++) begin
            @(posedge clk_core);
            @(negedge clk_core);
            if (!alloc_valid || alloc_drop_ind)
                alloc_ok = 1'b0;
            if ((alloc_pkt_head != (cell_idx == 0)) ||
                (alloc_pkt_tail != (cell_idx == 5))) begin
                alloc_ok = 1'b0;
            end
            alloc_addr_q[cell_idx] = alloc_cell_addr;
            enq_sof <= 1'b0;
            enq_eof <= (cell_idx == 4);
        end
        enq_req <= 1'b0;
        enq_eof <= 1'b0;
        check("LP-006c", "six cells are allocated and appended", alloc_ok);

        wait(`LLE_PATH.q_cell_cnt_q[0] == CNT_W'(6));
        check("LP-006d", "6-cell packet chain state is correct",
              (`LLE_PATH.q_head_q[0] == alloc_addr_q[0]) &&
              (`LLE_PATH.q_tail_q[0] == alloc_addr_q[5]) &&
              (`LLE_PATH.q_cell_cnt_q[0] == CNT_W'(6)) &&
              (`LLE_PATH.q_uni_pkt_backlog_q[0] == CNT_W'(1)) &&
              (`LLE_PATH.free_cnt_q == CNT_W'(CELL_NUM-6)));
        check_core_sleep_window("LP-006e", "sleep remains stable for 30 cycles after append", 30);

        // 3) 走链: 连续取出6个 cell，地址顺序必须与挂链顺序一致。
        deq_ok = 1'b1;
        @(negedge clk_core);
        deq_req <= 1'b1;
        deq_queue_id <= TC_W'(0);
        ...
        for (int cell_idx = 0; cell_idx < 6; cell_idx++) begin
            @(posedge clk_core);
            @(negedge clk_core);
            deq_addr_q[cell_idx] = deq_cell_addr;
            if (!deq_cell_valid || (deq_cell_addr != alloc_addr_q[cell_idx]))
                deq_ok = 1'b0;
            if ((deq_pkt_head != (cell_idx == 0)) ||
                (deq_pkt_tail != (cell_idx == 5))) begin
                deq_ok = 1'b0;
            end
        end
        deq_req <= 1'b0;
        check("LP-006f", "six-cell chain traversal succeeds in order", deq_ok);
        check("LP-006g", "queue is empty after traversal but cells are not freed",
              (`LLE_PATH.q_cell_cnt_q[0] == '0) &&
              (`LLE_PATH.q_uni_pkt_backlog_q[0] == '0) &&
              (`LLE_PATH.free_cnt_q == CNT_W'(CELL_NUM-6)));
        check_core_sleep_window("LP-006h", "sleep remains stable for 30 cycles after traversal", 30);

        // 4) 还链: 用走链返回的真实地址背靠背还回 free 链。
        recycle_ok = 1'b1;
        @(negedge clk_core);
        recycle_req <= 1'b1;
        recycle_cell_addr <= deq_addr_q[0];
        recycle_queue_id <= TC_W'(0);
        recycle_is_mcast <= 1'b0;
        for (int cell_idx = 0; cell_idx < 6; cell_idx++) begin
            @(posedge clk_core);
            @(negedge clk_core);
            if (!recycle_ack)
                recycle_ok = 1'b0;
            if (cell_idx < 5)
                recycle_cell_addr <= deq_addr_q[cell_idx+1];
        end
        recycle_req <= 1'b0;
        check("LP-006i", "all six traversed cells are recycled", recycle_ok);
        wait(`LLE_PATH.free_cnt_q == CNT_W'(CELL_NUM));
        check("LP-006j", "free chain count is fully restored",
              (`LLE_PATH.free_cnt_q == CNT_W'(CELL_NUM)) &&
              (u_dut.q_occupied_vec[0] == 1'b0));
        check_core_sleep_window("LP-006k", "sleep remains stable for 39 cycles after recycle", 39);
    endtask
