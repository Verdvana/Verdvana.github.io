...
- **建议配图 2（挂链时寄存器 + SRAM 值变化举例，交给 AI 生成 PNG，须附文首"AI 生成配图统一风格锚点"）**：
  以"向队列 Q5 连续挂 3 个 cell A、B、C"为例，分 3 帧展示 free 链寄存器、队列链寄存器 (head/tail/cnt) 与 SRAM 的值变化，并突出 **tail 不在当拍写 SRAM、而是暂存在 `q_tail_q` 寄存器**这一关键点。
  > *A clean flat-vector 3-frame step diagram titled "Enqueue: hanging cells A,B,C onto queue Q5", corporate slide style, arranged left-to-right as Frame1/Frame2/Frame3. In each frame show: (a) a small register group for Q5 = { q_head_q, q_tail_q, q_cell_cnt_q }, (b) free-chain register "free_head_q", (c) a "Next-Ptr SRAM" mini-table showing only rows that get written. Frame1 "enqueue A (empty queue)": q_head_q=A, q_tail_q=A, cnt=1, NO SRAM write (highlight text "empty queue: no old tail, nothing written to SRAM this cycle"). Frame2 "enqueue B": q_tail_q updates A→B in register, and SRAM row A.next=B is written now (relink old tail); cnt=2. Frame3 "enqueue C": q_tail_q updates B→C in register, SRAM row B.next=C written; cnt=3. Use deep-blue (#3253DC) for SRAM, sky-blue (#6CA0F6) for registers, cyan (#00C2DE) to highlight the just-changed value each frame, a dashed callout box in green (#3DBB6B) explaining "tail kept in register q_tail_q, SRAM relink happens on the NEXT enqueue". White background, dark-gray (#2E2E38) text and arrows, thin 1px rounded boxes, monospace signal names, minimalist, 16:9, no photo, no 3D.*
...
- **建议表格：挂链到 Q5 的寄存器/SRAM 值演进 (A→B→C)**

  | 拍 | 动作 | q_head_q | q_tail_q | q_cell_cnt | 本拍 SRAM 写 |
  |---|---|---|---|---|---|
  | 1 | 挂 A (空队列) | A | A | 1 | 无 (无旧 tail) |
  | 2 | 挂 B | A | B | 2 | SRAM[A].next=B |
  | 3 | 挂 C | A | C | 3 | SRAM[B].next=C |
