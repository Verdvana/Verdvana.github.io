...
module smmu_tb;
    ...
    task automatic run_remaining_cases();
        ...
        begin
            ...
            // force_all starts at rr_ptr=0, so empty q0/q1/q2 may complete
            // before q3 is selected. Keep force asserted until q3 is flushed.
            guard = 0;
            while ((lle_qcnt(3) != 0) && guard < 400) begin @(negedge clk); guard++; end
            cfg_age_force_all = 1'b0;
            repeat (4) @(negedge clk);
        ...
        begin
            ...
            // q2 gets 4 cells and is left idle -> should age.
            ...
            guard = 0;
            aging_seen = 1'b0;
            while (!agf_done_w && guard < 400) begin
                @(negedge clk);
                aging_seen |= irq_aging | aging_notify_w;
                guard++;
            end
            aging_seen |= irq_aging | aging_notify_w;
            repeat (4) begin
            ...
            chk_b("C20 irq_aging_seen", aging_seen, 1'b1);
